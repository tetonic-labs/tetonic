...
async fn durable_reservation_round_trip() {
    ...
    let shared = tetonic_memory::SharedStore::open(dir.path().join("lokai.db"), 1).unwrap();
...
async fn capability_expiry_blocks_dispatch_revalidation() {
    use tetonic_broker::dispatch::revalidate_before_dispatch;
    use tetonic_fabric_protocol::{CapabilityRegistry, WorkerCapabilities};
    ...
    compute.placement_decision.decision = TrustPlacementDecision::Eligible {
        targets: vec![tetonic_domain::EligibleTarget {
            worker_id: WorkerId::new("w1"),
            trust: tetonic_domain::WorkerTrust::OwnerControlledEstate,
        }],
        required_verification: tetonic_domain::VerificationRequirement::None,
    };
    let now = Utc::now();
    ...
    let process = AuthorizedProcessRequest {
        ...
        authorized_action: AuthorizedAction {
            capability: IssuedCapability {
                capability_id: tetonic_domain::CapabilityId::new("cap"),
