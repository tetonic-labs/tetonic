...
use tetonic_domain::{DataClass, SpeculationConfig, WorkerTrust};
use tetonic_fabric_protocol::{
    CapabilityRegistry, JobKind, SandboxCapabilities, WorkerCapabilities,
};
use tetonic_inference::{ChatRequest, FabricSnapshot, Message, NodeInfo, LOCAL_NODE_ID};
...
fn remote_reject_and_transport_failure_map_to_next_eligible() {
    ...
    let err = tetonic_inference::InferenceError::Provider("connection reset".into());
    ...
    let transport = tetonic_inference::InferenceError::Provider("transport timeout".into());
    assert_eq!(
...
fn worker_loss_and_hop_skip_keep_failover_open() {
    ...
    let lost = tetonic_inference::InferenceError::Preempted {
        node_id: "w1".into(),
    };
    assert_eq!(
    ...
    let hop_skip = tetonic_inference::InferenceError::Provider(
...
fn durable_scheduler_decision_round_trip() {
    ...
    let shared = tetonic_memory::SharedStore::open(&db, 1).expect("open");
...
fn broker_speculation_race_cancels_loser_late_result() {
    ...
    use tetonic_inference::ActiveJobRegistry;
