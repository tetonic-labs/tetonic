...
use chrono::Utc;
use tetonic_domain::ids::{AttemptId, ReservationId, WorkerId};
use tetonic_domain::sinks::{
    AuthorizedProcessRequest, ManagedProcessResult, ProcessBroker, ProcessBrokerError,
};
use tetonic_domain::{ProjectPlacementPolicy, SpeculationConfig, WorkerTrust};
use tetonic_fabric_protocol::{CapabilityRegistry, JobKind};
...
impl DefaultComputeBroker {
    ...
    pub async fn chat_admitted(
        ...
        tetonic_telemetry::record_compute_stage(
            tetonic_telemetry::span_names::COMPUTE_SUBMIT,
            Some("infer"),
            None,
            None,
            None,
        ...
        let schedule_timer = tetonic_telemetry::StageTimer::start_visible(
            tetonic_telemetry::PerfStage::InferenceSchedule,
        );
        let discovery_timer = tetonic_telemetry::StageTimer::start_visible(
            tetonic_telemetry::PerfStage::InferenceDiscovery,
        );
        ...
        let persist_timer = tetonic_telemetry::StageTimer::start_visible(
            tetonic_telemetry::PerfStage::InferenceSchedulePersist,
        );
        ...
        tetonic_telemetry::record_compute_stage(
            tetonic_telemetry::span_names::SCHEDULER_DECIDE,
            Some("infer"),
            None,
        ...
        let _ = adapter;
        let mut timing = tetonic_telemetry::CoordinatorObservedTiming::start(
    ...
    fn record_scheduler_metrics(&self, decision: &SchedulerDecision) {
        ...
        tetonic_telemetry::emit_safe_metric("scheduler_reason", decision.reason.as_str());
    ...
    pub fn on_worker_lost(&self, worker_id: &str) -> Vec<AttemptId> {
        ...
        if let Ok(mut g) = self.attempts.lock() {
            for rec in g.values_mut() {
                ...
                let secret = matches!(rec.request.data_class, tetonic_domain::DataClass::Secret);
    ...
    fn finish_attempt(&self, attempt_id: &AttemptId, ok: bool, err: Option<String>) {
        if let Some(res) = self.budgets.get_by_attempt(attempt_id) {
            ...
            tetonic_telemetry::record_compute_stage(
                tetonic_telemetry::span_names::RESOURCE_RELEASE,
                None,
                if ok {
                    Some("succeeded")
                } else {
    ...
    pub fn cancel_run_jobs(&self, run_id: &tetonic_domain::ids::RunId) {
        let _ = self
            .budgets
...
impl ComputeBroker for DefaultComputeBroker {
    async fn submit(&self, request: ComputeRequest) -> Result<ComputeHandle, ComputeBrokerError> {
        ...
        tetonic_telemetry::record_compute_stage(
            tetonic_telemetry::span_names::ADMISSION_EVALUATE,
    ...
    async fn cancel(
        ...
        if let (Some(sup), Some((run_id, task_id))) = (supervisor, pair) {
            let _ = tetonic_run::cancel_hop(sup.as_ref(), run_id, task_id).await;
        }
...
fn validate_request(request: &ComputeRequest) -> Result<(), ComputeBrokerError> {
    if matches!(
        ...
        tetonic_domain::TrustPlacementDecision::Denied { .. }
    ) {
