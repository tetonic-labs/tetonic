...
use tetonic_memory::{mutex_lock, SharedStore};
...
fn decode_target(raw: &str) -> ReservationTarget {
    if let Some(id) = raw.strip_prefix("worker:") {
        ReservationTarget::Worker {
            worker_id: tetonic_domain::WorkerId::new(id),
        }
    } else {
...
fn reservation_from_row(r: tetonic_memory::ComputeReservationRow) -> ResourceReservation {
    ResourceReservation {
...
mod tests {
    ...
    fn memory_reservation_survives_sqlite_reopen() {
        ...
        {
            let shared = tetonic_memory::SharedStore::open(&path, 1).unwrap();
