...
use super::contracts::*;
use lokai_domain::{AgentAttemptExecutor, CandidateOutcome};
...
impl super::service::ManagedRunService {
    pub async fn execute_attempt(
        ...
        let claimed = self
            .supervisor
            .handle(lokai_domain::RunCommand::ClaimExecution(
                lokai_domain::StartAttempt {
        ...
        let mut step_fn = |step: lokai_core::Step| {
            self.notify_hooks(|hooks| hooks.step(&binding, &step));
            on_step(step);
        };

        let mut executor =
            lokai_runtime::LocalAgentAttemptExecutor::new(agent, conversation, &mut step_fn);

        let ctx = lokai_domain::AttemptExecutionContext {
    ...
    pub async fn start_identity_job(
        ...
        let mut conversation = lokai_core::Conversation::new();
        let outcome = self
            .execute_attempt(
    ...
    pub async fn submit_identity_job(
        &self,
        cmd: StartIdentityJobCommand,
        mut agent: lokai_core::Agent,
    ) -> Result<
        (
            lokai_domain::AttemptId,
