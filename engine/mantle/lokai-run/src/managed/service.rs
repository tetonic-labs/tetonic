...
use lokai_domain::{AttemptId, ReplayGap, RunEventEnvelope, RunId, RunSnapshot};
...
pub struct ManagedRunService {
    ...
    pub(crate) store: Option<lokai_memory::SharedStore>,
    pub(crate) artifacts: Arc<dyn lokai_domain::artifact::ArtifactStore>,
    pub(crate) policy: Arc<lokai_policy::PolicyEngine>,
...
impl ManagedRunService {
    ...
    pub fn store(&self) -> Option<&lokai_memory::SharedStore> {
        self.store.as_ref()
    }

    pub fn policy(&self) -> &Arc<lokai_policy::PolicyEngine> {
        &self.policy
    }

    pub fn artifacts(&self) -> &Arc<dyn lokai_domain::artifact::ArtifactStore> {
        &self.artifacts
    }
    ...
    pub async fn abandon_recovery_run(
        ...
        if snapshot.state != lokai_domain::RunState::RecoveryRequired {
            return Err(ManagedRunError::InvalidRequest(
                "run does not require recovery".into(),
            ));
        }
        self.supervisor
            .handle(lokai_domain::RunCommand::CancelRun(
                lokai_domain::CancelRun {
    ...
    pub async fn cancel_run(&self, run_id: &RunId) -> Result<(), ManagedRunError> {
        let cmd = lokai_domain::CancelRun {
            envelope: crate::command_envelope(
        ...
        let result = self
            .supervisor
            .handle(lokai_domain::RunCommand::CancelRun(cmd))
            .await
            .map_err(|e| ManagedRunError::PersistenceFailed(e.to_string()));
        for active in actives {
            ...
            let outcome = match &result {
                Ok(_) => lokai_domain::CandidateOutcome::Canceled {
                    reason: "run canceled".into(),
                },
                Err(error) => lokai_domain::CandidateOutcome::Failed {
