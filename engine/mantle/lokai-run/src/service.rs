...
pub trait RunSupervisor: Send + Sync {
    ...
    async fn resume_from_sequence(
        ...
        after_sequence: u64,
        limit: u32,
    ) -> Result<Result<Vec<RunEventEnvelope>, lokai_domain::ReplayGap>, RunSupervisorError>;
...
impl DurableRunSupervisor {
    ...
    async fn handle_internal(
        ...
        let event = RunEventEnvelope {
            event_id: lokai_domain::ids::EventId::new(uuid::Uuid::new_v4().to_string()),
            run_id: next.run_id.clone(),
            sequence: next.sequence,
            event_type: EventType::Other(event_type_for(&command).to_string()),
            schema_version: 1,
            command_id: Some(lokai_domain::ids::CommandId::new(
                envelope.command_id.clone(),
            )),
            causation_id: None,
            correlation_id: Some(lokai_domain::ids::TraceId::new(
...
impl RunSupervisor for DurableRunSupervisor {
    ...
    async fn resume_from_sequence(
        ...
        if after_sequence > 0 && replay_floor > 0 && after_sequence < replay_floor {
            return Ok(Err(lokai_domain::ReplayGap {
                ...
                reason: lokai_domain::ReplayGapReason::OlderThanFloor,
            }));
        }
...
) -> lokai_domain::CommandEnvelope {
    lokai_domain::CommandEnvelope {
        command_id: command_id.into(),
        expected_sequence,
        trace: lokai_domain::TraceContext::default(),
        actor: actor.into(),
