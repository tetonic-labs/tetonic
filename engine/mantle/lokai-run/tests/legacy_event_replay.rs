...
use lokai_domain::{ExecutionTargetId, RunEventEnvelope};
use lokai_run::{empty_snapshot, replay_from_events, RunSupervisor};
...
fn legacy_local_holder_deserializes_as_local_variant() {
    let lease = harness::default_lease_cmd(
        lokai_run::command_envelope("c1", None, "test"),
        lokai_domain::RunId::new("run_1"),
        lokai_domain::AttemptId::new("att_1"),
    );
    let mut payload = serde_json::to_value(lokai_domain::RunCommand::LeaseAttempt(lease)).unwrap();
    payload["holder"] = serde_json::Value::String("local".into());

    let command = serde_json::from_value::<lokai_domain::RunCommand>(payload)
        .expect("legacy local holder must deserialize");
    let lokai_domain::RunCommand::LeaseAttempt(lease) = command else {
