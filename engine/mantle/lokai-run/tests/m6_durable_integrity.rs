...
use lokai_domain::{RunId, RunSnapshot, RunState, SessionId, TaskId, TaskRecord, TaskState};
use lokai_memory::SharedStore;
use lokai_run::{detect_recovery_required, DurableRunSupervisor, RunSupervisor};
...
fn seed(store: &SharedStore, snap: &RunSnapshot) {
    ...
    let event = lokai_domain::RunEventEnvelope {
        event_id: lokai_domain::ids::EventId::new("ev_seed"),
        run_id: snap.run_id.clone(),
        sequence: snap.sequence,
        event_type: lokai_domain::EventType::Other("seed".into()),
        schema_version: 1,
        command_id: None,
        causation_id: None,
        correlation_id: None,
        actor: lokai_domain::EventActor {
            name: "test".into(),
        },
        occurred_at: chrono::Utc::now(),
        recorded_at: chrono::Utc::now(),
        data_class: Default::default(),
        payload_digest: lokai_run::payload_digest::digest_event_payload(&payload).unwrap(),
        payload,
    };
    store
        .write_sync({
