...
async fn restart_during_task_states() {
    ...
    let session = lokai_domain::SessionId::new("sess_restart");
...
async fn stale_expected_run_sequence() {
    ...
    let (run, root, _) = create_started_run(&sup).await;
    let err = sup
        .handle(RunCommand::MarkTaskReady(lokai_domain::MarkTaskReady {
...
async fn artifact_rejection_refuses_cross_task_identity_and_missing_projection() {
    ...
    let mut cmd = lokai_domain::RejectArtifact {
    ...
    assert!(matches!(
        lokai_run::transition::apply_command(&snapshot, &RunCommand::RejectArtifact(cmd.clone())),
        Err(RunSupervisorError::Conflict(_))
    ));
    cmd.task_id = root.clone();
    snapshot.tasks.remove(&root);
    assert!(matches!(
        lokai_run::transition::apply_command(&snapshot, &RunCommand::RejectArtifact(cmd)),
