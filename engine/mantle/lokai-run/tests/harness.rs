...
use lokai_run::{binding_input_digest, command_envelope, DurableRunSupervisor, RunSupervisor};
...
pub fn db_supervisor() -> (DurableRunSupervisor, lokai_memory::SharedStore) {
...
pub async fn create_started_run(sup: &DurableRunSupervisor) -> (RunId, TaskId, SessionId) {
    ...
    .await
    .unwrap();
    sup.handle(RunCommand::StartRun(lokai_domain::StartRun {
...
pub async fn fail_dependency_task(sup: &DurableRunSupervisor, run: &RunId, dep: &TaskId) {
    ...
    fail_running_attempt(
        ...
        run,
        dep_attempt,
        lokai_domain::FailureClass::PermanentExecutionFailure,
        "fail",
    )
    .await;
}
...
pub async fn fail_running_attempt(
    ...
    run: &RunId,
    attempt_id: AttemptId,
    failure_class: lokai_domain::FailureClass,
    reason: &str,
) {
    let proof = lease_proof_from_snapshot(sup, run, &attempt_id).await;
...
pub async fn sup2_snapshot(
    ...
    run: &RunId,
) -> lokai_domain::RunSnapshot {
    let sup2 = DurableRunSupervisor::new(Some(store.clone()));
    sup2.snapshot(run.clone()).await.unwrap()
}

pub fn task_state(snap: &lokai_domain::RunSnapshot, task: &TaskId) -> TaskState {
