use lokai_domain::{RunCommand, RunSupervisorError, StorageLimits};
...
async fn test_quota_limits_enter_safe_mode() {
    ...
    let quotas = Arc::new(lokai_run::quotas::StorageQuotaManager::new(StorageLimits {
    ...
    for i in 0..40 {
        let res = sup
            .handle(RunCommand::CreateRun(lokai_domain::CreateRun {
                envelope: harness::env(None, &format!("create_{i}")),
                session_id: Some(lokai_domain::SessionId::new("s1")),
                run_id: lokai_domain::RunId::new(format!("r{i}")),
                root_task_id: lokai_domain::TaskId::new("t1"),
...
async fn test_safe_mode_blocking() {
    ...
    let res = sup
        .handle(RunCommand::CreateRun(lokai_domain::CreateRun {
            ...
            session_id: Some(lokai_domain::SessionId::new("s1")),
            run_id: lokai_domain::RunId::new("r1"),
            root_task_id: lokai_domain::TaskId::new("t1"),
            root_binding: Default::default(),
            speculation: None,
            job_spec: None,
...
async fn test_run_event_payload_digest_populated() {
    ...
    let run_a = lokai_domain::RunId::new("digest_a");
    let run_b = lokai_domain::RunId::new("digest_b");
    for (run, cid) in [(&run_a, "create_a"), (&run_b, "create_b")] {
        sup.handle(RunCommand::CreateRun(lokai_domain::CreateRun {
            envelope: harness::env(None, cid),
            session_id: Some(lokai_domain::SessionId::new("s_digest")),
            run_id: run.clone(),
            root_task_id: lokai_domain::TaskId::new("t1"),
            root_binding: Default::default(),
            speculation: None,
...
async fn compact_then_resume_below_floor_returns_gap() {
    ...
    for i in 0..3 {
        ...
        let task = lokai_domain::TaskId::new(format!("extra_{i}"));
        sup.handle(RunCommand::AddTask(lokai_domain::AddTask {
    ...
    let report = store
        .write({
            ...
            move |db| {
                ...
                let session = lokai_domain::SessionId::new("sess_1");
                let recovery = lokai_run::replay_from_events(
                    &lokai_run::empty_snapshot(lokai_domain::RunId::new(&run_id), Some(session)),
                    &below,
                )
                .expect("recovery");
    ...
    assert!(matches!(
        gap.reason,
        lokai_domain::ReplayGapReason::OlderThanFloor
    ));
...
fn test_migration_manager_backup_and_safe_mode_on_failure() {
    ...
    lokai_memory::Store::open(&db).expect("create db");
    let mgr = lokai_run::migration::MigrationManager::new();
    ...
    let mgr2 = lokai_run::migration::MigrationManager::new();
