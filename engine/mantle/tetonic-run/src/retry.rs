...
use tetonic_domain::{AttemptState, FailureClass, RunSnapshot, TaskState, TimeoutKind};
...
pub fn apply_failure_with_retry(
    ...
    out.tasks.get_mut(task_id).unwrap().retry = tetonic_domain::TaskRetryState {
        attempt_count,
...
pub fn verification_remediation(class: &FailureClass) -> tetonic_domain::VerificationRemediation {
    if matches!(class, FailureClass::VerificationFailed) {
        tetonic_domain::VerificationRemediation {
            retry_with_revised_input: true,
            spawn_remediation_task: true,
            require_different_specialist: false,
        }
    } else {
        tetonic_domain::VerificationRemediation::default()
    }
}
