...
use harness::*;
use tetonic_domain::{CancelRun, ClaimFinalization, RunCommand, RunSupervisorError, TaskState};
use tetonic_run::{try_claim_finalization, RunSupervisor};

async fn claim_cmd(
    sup: &tetonic_run::DurableRunSupervisor,
    run: &tetonic_domain::RunId,
    task: &tetonic_domain::TaskId,
...
async fn workfin01_claim_rejects_other_winner() {
    ...
    cmd.attempt_id = tetonic_domain::AttemptId::new("att_other");
