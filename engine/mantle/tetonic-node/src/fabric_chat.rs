...
use serde_json::json;
use tetonic_fabric_client::{build_signed_chat_result_timed, fabric_job_to_envelope};
...
pub(super) async fn jobs_cancel(
    ...
    if let Err(e) = tetonic_fabric_protocol::validate_cancellation(&cancel) {
        return json_response(
            StatusCode::BAD_REQUEST,
    ...
    let ack = CancellationAcknowledged {
        context: LifecycleContext {
            job_id: tetonic_domain::ids::JobId::new(job_id.as_deref().unwrap_or(&attempt_id)),
            ...
            worker_id: tetonic_domain::ids::WorkerId::new(&state.peer_id),
            sequence_number: 1,
            protocol_version: ProtocolVersion(tetonic_fabric_protocol::PROTOCOL_VERSION),
...
pub(super) async fn jobs_lease(
    ...
    if let Err(e) = tetonic_fabric_protocol::validate_lifecycle_context(
        &lease_req.context,
...
pub(super) async fn jobs(state: Arc<FabricState>, req: Request<Incoming>) -> Response<FabricBody> {
    ...
    if !matches!(
        ...
        tetonic_fabric_protocol::VersionedJobPayload::V1Infer(_)
    ) {
        return json_response(
            StatusCode::BAD_REQUEST,
    ...
    match state.job_ingress.accept(&job_envelope).await {
        ...
        Err(e) => {
            let status = if e.code == tetonic_fabric_protocol::FabricErrorCode::InternalFailure {
                StatusCode::INTERNAL_SERVER_ERROR
            } else {
                StatusCode::CONFLICT
            };
    ...
    let tetonic_fabric_protocol::VersionedJobPayload::V1Infer(ref payload) = job_envelope.payload
    else {
        return json_response(
            StatusCode::BAD_REQUEST,
            json!({"ok": false, "error": "missing V1Infer payload"}),
    ...
    let model = payload
        ...
        .unwrap_or("")
        .to_string();
    let tools: Vec<tetonic_inference::ToolSchema> =
    ...
    let job = FabricJob {
        ...
        messages,
        tools,
        options: tetonic_inference::SampleOptions {
            stream: Some(true),
            ..Default::default()
        },
        priority: tetonic_inference::JobPriority::OwnerInteractive,
        data_class: job_envelope.data_class,
        disclosure_tier: tetonic_inference::DisclosureTier::Auditable,
        audit_envelope: None,
        circle_id: None,
...
async fn chat_json(
    state: Arc<FabricState>,
    job: FabricJob,
    envelope: tetonic_fabric_protocol::JobEnvelope,
) -> Response<FabricBody> {
...
) -> Result<tetonic_inference::ChatResponse, ChatRunError> {
...
mod tests {
    ...
    fn test_state(inference: Arc<dyn InferenceProvider + Send + Sync>) -> Arc<FabricState> {
        ...
        let result_key_id = tetonic_fabric_client::key_id_from_public(&result_public_key);
        Arc::new(FabricState {
            ...
            cancel_state: Arc::new(tokio::sync::Mutex::new(
                tetonic_fabric_protocol::CancellationState::default(),
            )),
    ...
    async fn jobs_cancel_signals_scheduler_for_mapped_attempt() {
        use tetonic_domain::ids::{AttemptId, LeaseId, RunId, TaskId};
        use tetonic_fabric_protocol::{
        ...
        let mut rx = state
            .scheduler
            .acquire(job_id, tetonic_inference::JobPriority::OwnerInteractive)
            .await
            .unwrap();
        ...
        assert!(cs
            .acknowledge(
                ...
                CancellationAcknowledged {
                    context: tetonic_fabric_protocol::LifecycleContext {
                        job_id: tetonic_domain::ids::JobId::new(job_id),
                        task_id: TaskId::new(attempt_id),
                        attempt_id: AttemptId::new(attempt_id),
                        lease_id: LeaseId::new(attempt_id),
                        lease_epoch: 1,
                        worker_id: tetonic_domain::ids::WorkerId::new("test_peer"),
                        sequence_number: 1,
                        protocol_version: tetonic_fabric_protocol::ProtocolVersion(1),
    ...
    async fn scheduler_refuse_after_accept_is_not_duplicate_in_flight() {
        ...
        let job = FabricJob {
            ...
            tools: vec![],
            options: Default::default(),
            priority: tetonic_inference::JobPriority::CircleBackground,
            data_class: DataClass::RepositorySource,
            disclosure_tier: Default::default(),
            audit_envelope: None,
            circle_id: None,
