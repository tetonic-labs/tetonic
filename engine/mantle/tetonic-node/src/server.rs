...
use tetonic_enroll::DEFAULT_FABRIC_PORT;
...
pub struct FabricServer {
    log: IngressLog,
    keys: Arc<dyn tetonic_domain::key_storage::KeyStorage>,
    scheduler: Arc<WorkerScheduler>,
}

impl FabricServer {
    pub fn new() -> Self {
        Self::with_key_storage(Arc::new(tetonic_secrets::key_storage::PlatformKeyStorage))
    }

    pub fn with_key_storage(keys: Arc<dyn tetonic_domain::key_storage::KeyStorage>) -> Self {
        Self {
            keys,
...
mod tests {
    ...
    use tetonic_inference::{DataClass, FabricJob, JobPriority};
    use tetonic_memory::WorkerStore;
    ...
    async fn spawn_test_server(
        ...
        store
            .publish_tls_identity(
                None,
                &cert,
                &tetonic_domain::key_storage::SecretKeyRef("test-only".into()),
            )
            .unwrap();
