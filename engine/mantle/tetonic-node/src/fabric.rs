...
use rand::rngs::OsRng;
use serde_json::json;
use tetonic_capacity::worker_capacity_wire;
use tetonic_domain::ids::{KeyId, WorkerId};
use tetonic_egress::EgressGuard;
use tetonic_fabric_client::{key_id_from_public, OwnerActivityRequest};
...
use tetonic_inference::{DataClass, InferenceProvider, OllamaProvider};
use tetonic_memory::WorkerStore;
use tetonic_sandbox::platform_backend;
...
async fn capacity_status(state: &FabricState) -> Response<FabricBody> {
    ...
    let client: Arc<dyn tetonic_capacity::InferenceClient> = Arc::new(
        tetonic_capacity::OllamaInferenceClient::new(&ollama_base, state.ollama.clone()),
    );
    ...
    let (status, _) =
        tetonic_capacity::capacity_status_for_worker_path(db_path, client, ollama_version).await;
...
async fn revoke(state: Arc<FabricState>, req: Request<Incoming>) -> Response<FabricBody> {
    ...
    let db_result = tokio::task::spawn_blocking(move || {
        ...
        Ok::<(), tetonic_memory::WorkerStoreError>(())
    })
    .await;
...
fn negotiate_from_body(
    body: &[u8],
) -> Result<tetonic_fabric_protocol::VersionNegotiationResponse, tetonic_fabric_protocol::FabricError>
{
    use tetonic_fabric_protocol::{
