...
pub(crate) fn index_mentions_path(
    opener: &dyn tetonic_domain::CodeIndexOpen,
    db: &std::path::Path,
...
mod tests {
    ...
    fn indexed_file_hint_uses_storage_key() {
        ...
        let index = tetonic_index::Index::open(&db).unwrap();
        index.index_workspace(ws).unwrap();
        std::fs::remove_file(ws.join("widget.rs")).unwrap();
        let opener = tetonic_index::FilesystemCodeIndex;
        let key = tetonic_index::workspace_storage_key(ws);
        assert!(
