...
use lokai_inference::{ChatRequest, InferenceError};
use lokai_memory::new_id;
...
impl DefaultComputeBroker {
    ...
    pub(crate) async fn bind_hop_attempt(
        ...
        if !lokai_run::hop_run_classified(&snap) {
    ...
    async fn supervisor_create_and_lease(
        ...
        if let Some(task) = snap.tasks.get(&compute_req.task_id) {
            if !lokai_run::hop_task_leaseable(&task.state, snap.speculation.allowed) {
                return Err(InferenceError::Provider(format!(
        ...
        let created_seq = lokai_run::create_hop_attempt(
            supervisor.as_ref(),
            compute_req.run_id.clone(),
            compute_req.task_id.clone(),
            attempt_id.clone(),
        ...
        let leased = lokai_run::lease_hop(
            supervisor.as_ref(),
            compute_req.run_id.clone(),
            attempt_id.clone(),
...
mod tests {
    ...
    use lokai_run::command_envelope;
    ...
    fn chat() -> ChatRequest {
        ChatRequest {
            ...
            draft_count: None,
            keep_alive: None,
            fabric: Some(lokai_inference::FabricCallMeta {
        ...
        assert!(
            matches!(
                ...
                lokai_domain::AttemptState::Failed
                    ...
                    | lokai_domain::AttemptState::LeaseExpired
            ),
    ...
    async fn additional_hop_leases_second_attempt_without_failing_primary() {
        ...
        assert!(
            matches!(
                primary.state,
                lokai_domain::AttemptState::Leased
                    | lokai_domain::AttemptState::Created
                    | lokai_domain::AttemptState::Starting
