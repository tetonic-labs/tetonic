...
use lokai_domain::secrets::{OutboundRedaction, OutboundRedactionSink, ScanContext, SecretScanner};
use lokai_inference::{
...
impl InferenceProvider for BrokerInferenceProvider {
    async fn chat(
        ...
        let timer =
            lokai_telemetry::StageTimer::start_visible(lokai_telemetry::PerfStage::InferenceScan);
...
mod outbound_redaction_tests {
    use super::*;
    use lokai_domain::secrets::{RedactionRecordReference, ScanOutcome};
    use lokai_domain::workspace::ContentDigest;
    use lokai_inference::{FabricCallMeta, Message, ToolCall};
    ...
    async fn outbound_exception_is_bound_to_request_session() {
        ...
        let fingerprint =
            lokai_secrets::detectors::default_detectors()[1].scan(secret, None, &scanner.hmac_key)
                [0]
            .fingerprint
            .0
            .clone();
        scanner.grant_override(
            &fingerprint,
            lokai_secrets::OverrideScope::Session("allowed".into()),
        );
