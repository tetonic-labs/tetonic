...
use tetonic_domain::{ProjectPlacementPolicy, WorkerTrust};
...
use tetonic_inference::evaluate_typed_job_placement;
...
pub fn observe_heartbeat(
    ...
    limits: &tetonic_fabric_protocol::MessageSizeLimits,
) -> Result<(), FabricClientError> {
    tetonic_fabric_protocol::validate_heartbeat(hb, channel_worker_id, limits)
...
pub fn request_lease_renewal(
    ...
    tetonic_fabric_protocol::validate_lifecycle_context(&req.context, &req.context.worker_id)
        .map_err(map_protocol_err)?;
...
pub fn deliver_cancellation(
    ...
    tetonic_fabric_protocol::validate_cancellation(cancel).map_err(map_protocol_err)
}
...
mod tests {
    use super::*;
    use tetonic_domain::ids::{AttemptId, JobId, LeaseId, TaskId, WorkerId};
    use tetonic_fabric_protocol::{IdempotencyKey, LifecycleContext, ProtocolVersion};
    ...
    fn legacy_cancellation_delivery_rejected() {
        ...
        let cancel = CancellationRequest {
            run_id: tetonic_domain::ids::RunId::new("run"),
    ...
    fn lease_renewal_rejected_for_legacy_worker() {
        ...
        let job = crate::build_infer_job_envelope(
            ...
            &[tetonic_inference::Message::user("hello")],
            "qwen:7b",
        )
        .unwrap();
    ...
    fn lease_renewal_rechecks_trust_after_downgrade() {
        ...
        let mut job = crate::build_infer_job_envelope(
            ...
            &[tetonic_inference::Message::user("sensitive source")],
            "qwen:7b",
        )
        .unwrap();
        job.data_class = tetonic_domain::DataClass::SensitiveSource;
        let worker_id = WorkerId::new("worker");
        let now = Utc::now();
        let mut worker_caps = tetonic_fabric_protocol::WorkerCapabilities::legacy_infer_profile(
            worker_id.clone(),
            "boot",
            1,
            0,
