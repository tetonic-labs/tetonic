...
pub use tetonic_fabric_protocol::{
...
pub fn validate_outbound_job(
    ...
    let payload_len = serde_json::to_vec(&envelope.payload.job)
        .map(|b| b.len())
        .unwrap_or(0);
    tetonic_fabric_protocol::check_payload_size(payload_len, limits).map_err(map_protocol_err)?;
    Ok(())
}
...
pub fn encode_job_offer_envelope(
    coordinator_id: tetonic_fabric_protocol::CoordinatorId,
    worker_id: tetonic_domain::ids::WorkerId,
    job: JobEnvelope,
    revocation_epoch: u64,
) -> FabricEnvelope<JobOffer> {
    FabricEnvelope {
        protocol_version: ProtocolVersion(tetonic_fabric_protocol::PROTOCOL_VERSION),
        message_id: tetonic_fabric_protocol::MessageId(format!("msg_{}", uuid_simple())),
        message_type: FabricMessageType::JobOffer,
        coordinator_id,
        worker_id,
        sent_at: Utc::now(),
        revocation_epoch,
        trace_context: tetonic_fabric_protocol::FabricTraceContext {
...
pub fn encode_lease_renewal_envelope(
    coordinator_id: tetonic_fabric_protocol::CoordinatorId,
    worker_id: tetonic_domain::ids::WorkerId,
    lease: tetonic_fabric_protocol::LeaseRenewalRequest,
    revocation_epoch: u64,
) -> FabricEnvelope<tetonic_fabric_protocol::LeaseRenewalRequest> {
    FabricEnvelope {
        protocol_version: ProtocolVersion(tetonic_fabric_protocol::PROTOCOL_VERSION),
        message_id: tetonic_fabric_protocol::MessageId(format!("msg_{}", uuid_simple())),
