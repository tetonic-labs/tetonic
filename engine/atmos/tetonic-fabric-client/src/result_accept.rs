...
use tetonic_artifact::quarantine::{
...
use tetonic_domain::result_integrity::{ArtifactOrigin, WorkerBehaviorSignals};
...
pub struct RemoteResultAcceptRequest {
    ...
    pub command_envelope: Option<tetonic_domain::CommandEnvelope>,
}
...
fn finish_reject(
    ...
    tetonic_telemetry::record_retained_outcome(
    ...
    for art in &req.envelope.body.artifacts {
        ...
        validate_remote_artifact_with(&mut meta, &art.digest, &limits, &inspection).map_err(
            |e| match e {
                ...
                tetonic_domain::artifact::ArtifactError::QuarantineRejected(msg)
