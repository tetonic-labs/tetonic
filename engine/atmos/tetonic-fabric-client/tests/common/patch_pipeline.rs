...
use tetonic_domain::workspace::{ContentDigest, PatchApproval, WorkspaceVersion};
use tetonic_domain::{DataClass, TransactionId, VerificationRecord};
use tetonic_fabric_protocol::ResultEnvelope;
use tetonic_transaction::{SecurityLimits, WorkspaceTransactionService, WorkspaceTxnConfig};

use tetonic_fabric_client::validate_patch_acceptance;
use tetonic_fabric_client::FabricClientError;
...
pub fn apply_verified_remote_patch(
    ...
    for (path, bytes) in &req.touched_paths {
        if !tetonic_artifact::is_safe_workspace_path(path) {
            return Err(FabricClientError::Http(format!(
                "unsafe patch path: {path}"
            )));
        }
        if tetonic_artifact::looks_executable(bytes, Some(path)) {
            return Err(FabricClientError::Http(format!(
