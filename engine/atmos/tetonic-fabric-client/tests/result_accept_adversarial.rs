...
use tetonic_domain::{ResultDisposition, ResultVerificationRequirement, TransactionId};
use tetonic_fabric_client::{
...
fn apply_verified_remote_patch_stages_and_commits() {
    ...
    let ws = tetonic_transaction::version::capture_workspace_version(dir.path(), &[])
