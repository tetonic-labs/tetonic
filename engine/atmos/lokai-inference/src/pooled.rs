...
use lokai_fabric_protocol::{CapabilityRegistry, ModelSelection};
...
use lokai_domain::placement::{PlacementDecision, ProjectPlacementPolicy};
use lokai_policy::trust_permits_data_class;
...
pub type WorkerTrustResolver =
    dyn Fn(&str) -> Option<(lokai_domain::WorkerTrust, u64)> + Send + Sync;
...
impl PooledProvider {
    ...
    pub fn on_trust_change(&self, worker_id: &str, trust: lokai_domain::WorkerTrust) {
    ...
    pub fn worker_trust_for(&self, worker_id: &str) -> Option<lokai_domain::WorkerTrust> {
        self.remotes
            .iter()
    ...
    fn resolved_remote_trust(remote: &dyn FabricNodeProvider) -> Option<lokai_domain::WorkerTrust> {
    ...
    fn revalidate_worker_at_dispatch(
        ...
        attempt: &ChatRequest,
        worker_id: &str,
        trust: lokai_domain::WorkerTrust,
        model: &ModelSelection,
        project_policy: &ProjectPlacementPolicy,
    ) -> Result<(), lokai_domain::PlacementReason> {
    ...
    pub fn cancel_session_jobs(&self, session_id: &str) {
        ...
        for remote in &self.remotes {
            ...
            for (job_id, attempt_id, stored_run_id) in &jobs {
                ...
                let cancel = lokai_fabric_protocol::CancellationRequest {
                    run_id: lokai_domain::ids::RunId::new(run_id),
                    task_id: lokai_domain::ids::TaskId::new(attempt_id),
                    attempt_id: lokai_domain::ids::AttemptId::new(attempt_id),
                    lease_id: lokai_domain::ids::LeaseId::new(attempt_id),
                    lease_epoch: 1,
        ...
        if let Some(reg) = &self.capability_registry {
            if let Ok(guard) = reg.read() {
                if let Some(caps) = guard.get(&node.id) {
                    return lokai_fabric_protocol::model_inventory_matches(
    ...
    fn placement_order(
        ...
        if data_class == lokai_domain::DataClass::Secret {
...
mod tests {
    ...
    fn local_preferred_when_model_available() {
        ...
        let order = pooled.placement_order(
            ...
            None,
            &[],
            lokai_domain::DataClass::default(),
        );
    ...
    fn broker_preferred_worker_placed_first() {
        ...
        impl crate::FabricNodeProvider for MockProvider {
            ...
            fn fabric_capabilities(&self) -> lokai_fabric_protocol::WorkerCapabilityAdvertisement {
                lokai_fabric_protocol::WorkerCapabilityAdvertisement::legacy_v1_chat_infer_only()
            }
    ...
    fn quarantined_worker_excluded_from_placement() {
        ...
        use lokai_fabric_protocol::{SandboxCapabilities, WorkerCapabilities};
    ...
    fn degraded_worker_deprioritized_in_placement() {
        use lokai_domain::ids::WorkerId;
        use lokai_fabric_protocol::{ProbeSessionInput, WorkerCapabilities};
    ...
    fn trust_downgrade_blocks_sensitive_at_dispatch_revalidation() {
        ...
        assert_eq!(
            pooled
                ...
                .unwrap_err(),
            lokai_domain::PlacementReason::WorkerTrustInsufficient
        );
    }
    ...
    fn stale_revocation_epoch_blocks_dispatch_revalidation() {
        use crate::{ChatRequest, FabricCallMeta, Message};
        use lokai_domain::{ids::WorkerId, DataClass, WorkerTrust};
        ...
        assert_eq!(
            pooled
                ...
                .unwrap_err(),
            lokai_domain::PlacementReason::WorkerRevoked
        );
    }
    ...
    fn fallback_order_walks_only_listed_targets() {
        ...
        let order = pooled.placement_order(
            ...
            None,
            &order_labels,
            lokai_domain::DataClass::RepositorySource,
        );
        assert_eq!(
            order,
