...
fn legacy_worker_profile() {
    ...
    assert!(caps.supports_job_kind(&tetonic_fabric_protocol::JobKind::Infer));
}
...
fn expired_advertisement_fails_validation() {
    ...
    assert!(matches!(
        err.code,
        tetonic_fabric_protocol::FabricErrorCode::InvalidEnvelope
    ));
}
...
fn gpu_disappears_after_advertisement() {
    ...
    reg.upsert_probe_session(tetonic_fabric_protocol::ProbeSessionInput {
        caps: current,
...
fn coordinator_observed_health_failure() {
    let caps = sample_caps();
    let results = tetonic_fabric_protocol::run_coordinator_observed_probes(&caps, false, true);
    assert!(results
        .iter()
...
fn model_digest_drift_detected() {
    ...
    let results = tetonic_fabric_protocol::run_capability_drift_probes(&previous, &current);
    assert!(results
        .iter()
...
fn revocation_epoch_stale() {
    ...
    assert!(matches!(
        err.code,
        tetonic_fabric_protocol::FabricErrorCode::StaleRevocationEpoch
    ));
}
...
fn capability_cache_after_worker_software_upgrade() {
    ...
    assert_eq!(
        ...
        tetonic_fabric_protocol::CapabilityRefreshOutcome::Updated
    );
    assert_eq!(
