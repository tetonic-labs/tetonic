...
use tetonic_rpc::outbound::{OutboundClass, OutboundQueue};
...
async fn oversized_header_is_rejected_before_body() {
    ...
    assert!(tetonic_rpc::framing::read_frame(&mut reader).await.is_err());
}
...
async fn yielding_turn_retains_its_own_trace() {
    ...
    let a = tetonic_telemetry::propagation::scope_context(Default::default(), async {
        tetonic_telemetry::inject_turn_context("session_a", "run_a", None);
        ready_tx.send(()).unwrap();
        resume_rx.await.unwrap();
        tetonic_telemetry::propagation::extract_context().unwrap()
    });
    let b = tetonic_telemetry::propagation::scope_context(Default::default(), async {
        ready_rx.await.unwrap();
        tetonic_telemetry::inject_turn_context("session_b", "run_b", None);
