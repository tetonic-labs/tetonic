...
use lokai_inference::{FabricJobResult, GenUsageSerde, JobStatus, Message};
...
fn clean_scan() -> lokai_artifact::ScanPolicy {
    lokai_artifact::ScanPolicy::Scan(std::sync::Arc::new(|_| false))
}
...
async fn worker_revoked_before_dispatch_rejects_legacy_chat() {
    use lokai_inference::FabricNodeProvider;
...
async fn dispatch_revalidation_rejects_changed_workspace() {
    ...
    let store: Arc<dyn ArtifactStore> = Arc::new(
        lokai_artifact::LocalArtifactStore::new(dir.path().join("artifacts"), clean_scan())
            .unwrap(),
    );
    std::fs::write(dir.path().join("source.txt"), "before").unwrap();
    let expected = lokai_transaction::version::capture_workspace_version(dir.path(), &[]).unwrap();
...
async fn dispatch_revalidation_rejects_changed_artifact_digest() {
    use lokai_domain::artifact::{ArtifactDeclaration, ArtifactKind, RetentionPolicy};
    use lokai_domain::{AttemptId, RunId, TaskId};
    ...
    job.input_artifacts = vec![ArtifactReference {
        ...
        digest: lokai_domain::ContentDigest::new("sha256:stale"),
    }];
...
async fn remote_chat_refuses_unstamped_request() {
    use lokai_inference::{ChatRequest, FabricCallMeta, InferenceError, InferenceProvider};
