...
use lokai_domain::{CommandEnvelope, LeaseProof, ResultVerificationRequirement};
...
impl RemoteNodeProvider {
    ...
    pub(crate) async fn accept_signed_chat_result_with_redundant(
        ...
        let flags = registry
            ...
            .unwrap_or(lokai_inference::AttemptBindingFlags {
                known: true,
        ...
        let expected = ExpectedResultBinding {
            ...
            task_active: run_snapshot
                ...
                .map(|t| t.state != lokai_domain::TaskState::Canceled)
                .unwrap_or(true),
        ...
        let (outcome, signals_snapshot, durable, verification_ms) = {
            ...
            lokai_telemetry::record_compute_stage(
                lokai_telemetry::span_names::VERIFICATION,
                Some("infer"),
            ...
            let durable = ResultDispositionRecord {
                ...
                audit_only: matches!(
                    outcome.disposition,
                    lokai_domain::ResultDisposition::RejectedStaleAttempt
                        | lokai_domain::ResultDisposition::Superseded
                        | lokai_domain::ResultDisposition::RejectedCanceled
                ),
            };
            (outcome, signals_snapshot, durable, verification_ms)
        };

        let accepted = outcome.disposition == lokai_domain::ResultDisposition::Accepted;
        ...
        if accepted {
            if let Some(reg) = registry {
                if let Err(e) = reg.validate(&job_id, &attempt_id) {
                    durable.disposition = lokai_domain::ResultDisposition::RejectedStaleAttempt;
    ...
    fn apply_behavior_signal_eligibility(&self) {
        ...
        if let Ok(mut reg) = self.capability_registry.write() {
            match state {
                ...
                | lokai_domain::WorkerOperationalState::Revoked => {
                    reg.force_quarantine(&self.worker_id);
                }
                lokai_domain::WorkerOperationalState::Degraded => {
                    reg.force_degraded(&self.worker_id);
                }
                lokai_domain::WorkerOperationalState::Healthy => {}
            }
        }
    }
...
mod r2_tests {
    ...
    use lokai_domain::{CommandEnvelope, LeaseProof, ResultDisposition, RunSnapshot};
    use lokai_inference::{ActiveJobRegistry, Message};
    use rand::rngs::OsRng;
    ...
    impl DispositionPersistence for MemPersist {
        ...
        fn persist_behavior_signals(
            &self,
            _worker_id: &str,
            _signals: &lokai_domain::result_integrity::WorkerBehaviorSignals,
        ) -> Result<(), String> {
            Ok(())
        }
    ...
    fn signed_chat(
        ...
        lokai_fabric_protocol::JobEnvelope,
        serde_json::Value,
        String,
    ) {
        ...
        let job = build_infer_job_envelope(
            &LegacyInferParams {
                ...
                data_class: lokai_inference::DataClass::RepositorySource,
        ...
        let (result, env) = build_signed_chat_result(
            ...
            lokai_inference::GenUsageSerde::default(),
            lokai_inference::JobStatus::Ok,
            None,
        )
        .unwrap();
    ...
    async fn coordinator_epoch_rejects_stale_envelope() {
        ...
        let err = p
            .accept_signed_chat_result(
                ...
                json,
                None,
                lokai_domain::ResultVerificationRequirement::default(),
                None,
                None,
                None,
            )
    ...
    async fn accept_loads_lease_proof_for_job_attempt_id() {
        let looked = Arc::new(LookupBridge {
            ...
            proof: LeaseProof {
                lease_id: lokai_domain::ids::LeaseId::new("lease_r2"),
                lease_epoch: 0,
                holder: lokai_domain::ExecutionTargetId::worker("w1"),
            },
        });
        ...
        impl RemoteResultRunBridge for SnapBridge {
            async fn load_snapshot(&self, run_id: &str) -> Result<Option<RunSnapshot>, String> {
                let _ = run_id;
                Ok(Some(RunSnapshot {
                    run_id: lokai_domain::ids::RunId::new("run_r2"),
                    session_id: Some(lokai_domain::ids::SessionId::new("sess")),
                    state: lokai_domain::RunState::Active,
                    sequence: 1,
