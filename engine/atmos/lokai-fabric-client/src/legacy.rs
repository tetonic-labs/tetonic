...
use lokai_domain::ResultVerificationRequirement;
use lokai_domain::{artifact::ArtifactStore, ids::WorkerId, WorkerTrust};
...
impl RemoteNodeProvider {
    ...
    async fn revalidate_dispatch_state(
        &self,
        job: &lokai_fabric_protocol::JobEnvelope,
        ...
        if let Some(expected) = &job.workspace_version {
            ...
            let actual = tokio::task::spawn_blocking(move || {
                lokai_transaction::version::capture_workspace_version(&root, &[])
            })
            .await
            .map_err(|error| {
        ...
        if !job.input_artifacts.is_empty() {
            ...
            for expected in &job.input_artifacts {
                let id = lokai_domain::ids::ArtifactId::new(&expected.artifact_id);
    ...
    pub fn spawn_jobs_cancel(&self, cancel: CancellationRequest) {
        ...
        let envelope = encode_cancellation_envelope(
            lokai_fabric_protocol::CoordinatorId(self.estate_id.clone()),
            worker_id,
            cancel,
...
impl FabricNodeProvider for RemoteNodeProvider {
    ...
    fn request_jobs_cancel(&self, cancel: lokai_fabric_protocol::CancellationRequest) {
    ...
    async fn probe_node(&self) -> Option<NodeInfo> {
        ...
        if let Ok(r) = &caps {
            if r.status == 200
                && lokai_fabric_protocol::validate_capability_document_bytes(r.body.as_bytes())
                    .is_ok()
            {
                    ...
                    if let Some(doc) = parsed.worker_capabilities {
                        ...
                        if let Ok(mut reg) = self.capability_registry.write() {
                            ...
                            let outcome = reg.upsert_probe_session(
                                lokai_fabric_protocol::ProbeSessionInput {
                                    caps: doc,
    ...
    async fn chat_on_fabric(
        ...
        if let Some(meta) = fabric {
            ...
            protocol_job.input_artifacts = meta
                ...
                .map(|artifact| ArtifactReference {
                    ...
                    digest: lokai_domain::workspace::ContentDigest::new(&artifact.digest),
                })
                .collect();
            let wants_net = meta
        ...
        if let lokai_fabric_protocol::VersionedJobPayload::V1Infer(payload) =
        ...
        let required_result_verification = match &typed_placement {
            lokai_domain::placement::PlacementDecision::Eligible {
                required_verification:
                    lokai_domain::placement::VerificationRequirement::Required { policy_id },
                ..
            } => requirement_from_placement(true, policy_id.as_deref()),
            _ => ResultVerificationRequirement::StructuralValidation,
        };
        let worker_id = lokai_domain::ids::WorkerId::new(&self.worker_id);
        ...
        if !legacy_v1_chat_only {
            ...
            let lease_task = tokio::spawn(async move {
                ...
                loop {
                    ...
                    let lease_req = LeaseRenewalRequest {
                        context: LifecycleContext {
                            ...
                            protocol_version: ProtocolVersion(
                                lokai_fabric_protocol::PROTOCOL_VERSION,
                            ),
                            idempotency_key: IdempotencyKey(format!(
                                "lease:{}:{}:{}",
                    ...
                    let lease_env = encode_lease_renewal_envelope(
                        lokai_fabric_protocol::CoordinatorId(lease_estate.clone()),
                        lease_worker.clone(),
                        lease_req,
                        lease_epoch_policy,
                    );
            ...
            lokai_telemetry::record_compute_stage(
                lokai_telemetry::span_names::TRANSFER_INPUT,
                Some("infer"),
                None,
                None,
                Some("remote"),
            ...
            lokai_telemetry::record_compute_stage(
                lokai_telemetry::span_names::TRANSFER_OUTPUT,
                Some("infer"),
                None,
                None,
                Some("remote"),
...
struct FinishChatArgs<'a> {
    ...
    protocol_job: lokai_fabric_protocol::JobEnvelope,
