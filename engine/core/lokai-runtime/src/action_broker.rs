...
use lokai_core::{ApprovalHook, ApprovalRequest};
use lokai_domain::sinks::ActionBroker;
use lokai_domain::{
...
pub struct RuntimeActionBroker {
    ...
    attempt_approvals:
        std::sync::Mutex<std::collections::HashMap<lokai_domain::AttemptId, ApprovalHook>>,
}
...
impl RuntimeActionBroker {
    ...
    pub fn unregister_attempt_approval(&self, attempt_id: &lokai_domain::AttemptId) {
        self.attempt_approvals.lock().unwrap().remove(attempt_id);
    }

    pub fn has_attempt_approval(&self, attempt_id: &lokai_domain::AttemptId) -> bool {
        self.attempt_approvals
            .lock()
            .unwrap()
...
impl ActionBroker for RuntimeActionBroker {
    async fn evaluate_and_issue(
        ...
        let cap = IssuedCapability {
            ...
            max_use_count: 1,
            current_use_count: 0,
            issuing_policy_version: lokai_policy::policy_version().to_string(),
...
fn broker_tool_name(kind: &lokai_domain::ActionKind) -> String {
    match kind {
        lokai_domain::ActionKind::ExecuteShell => "run_shell".into(),
        lokai_domain::ActionKind::WriteFile => "write_file".into(),
        lokai_domain::ActionKind::ReadFile => "read_file".into(),
        lokai_domain::ActionKind::ExecuteProcess => "verify".into(),
        lokai_domain::ActionKind::StartInternalService => "lsp".into(),
        other => format!("{other:?}"),
    }
}
