...
fn lokai_core_has_no_index_or_lsp_in_resolve_graph() {
    let hits = package_depends_on_index_or_lsp("lokai-core").expect("cargo metadata");
    assert!(
        hits.is_empty(),
        "lokai-core resolve graph must not include lokai-index/lokai-lsp, found {hits:?}"
    );
}

#[test]
fn lokai_runtime_has_no_index_or_lsp_in_resolve_graph() {
    let hits = package_depends_on_index_or_lsp("lokai-runtime").expect("cargo metadata");
    assert!(
        hits.is_empty(),
        "lokai-runtime resolve graph must not include lokai-index/lokai-lsp, found {hits:?}"
    );
}
