...
async fn run_scenario(
    ...
    scenario: &str,
    limit: Duration,
) -> Result<tetonic_sandbox::SandboxRunResult, SandboxError> {
    let ws = workspace();
...
async fn adversarial_secret_env_not_inherited() {
    ...
    let script = "echo $LOKAI_TEST_SECRET";
    #[cfg(windows)]
    let req = tetonic_sandbox::apply_shell(req, "cmd", script);
    #[cfg(not(windows))]
    let req = tetonic_sandbox::apply_shell(req, "sh", script);
    let backend = platform_backend();
...
async fn adversarial_build_profile_reports_partial_network() {
    ...
    assert!(matches!(
        req.network,
        tetonic_sandbox::NetworkPolicy::DenyAll
    ));
    if !caps.network_denial {
        ...
        assert!(
            matches!(
                r.report.outcome,
                tetonic_sandbox::IsolationOutcome::BrokeredWithWarning { .. }
                    | tetonic_sandbox::IsolationOutcome::Sandboxed { .. }
            ),
