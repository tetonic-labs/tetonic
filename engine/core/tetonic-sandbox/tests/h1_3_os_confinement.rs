...
async fn deny_all_either_enforces_or_reports_missing() {
    ...
    let missing = tetonic_sandbox::backend::collect_missing_controls(&req, &caps);
    let has_gap = missing
        .iter()
...
async fn standard_profile_still_brokers_high_gap() {
    ...
    let missing = vec![tetonic_sandbox::MissingControl {
        control: SandboxControl::NetworkDenial,
        reason: "synthetic".into(),
        risk_level: tetonic_sandbox::RiskLevel::High,
    }];
    let outcome = tetonic_sandbox::backend::evaluate_outcome(&caps, &req, Vec::new(), missing)
...
async fn windows_strict_deny_all_either_enforces_or_denies() {
    ...
    req.network = tetonic_sandbox::NetworkPolicy::DenyAll;
...
async fn windows_fs_confinement_either_enforces_or_reports_missing() {
    ...
    let is_denied = matches!(res, Err(tetonic_sandbox::SandboxError::Denied(_)));
    assert!(
        is_denied,
