...
pub struct Agent {
    work_scope: lokai_domain::work_scope::WorkScope,
    ...
    action_broker: Option<Arc<dyn lokai_domain::sinks::ActionBroker>>,
    process_broker: Option<Arc<dyn lokai_domain::sinks::ProcessBroker>>,
...
impl ToolHost for EmptyToolHost {
    ...
    fn propose(&self, _name: &str, _args: &Value) -> Option<lokai_domain::tool_host::ToolProposal> {
        None
    }
    ...
    fn advertisements(&self) -> Vec<lokai_domain::tool_host::ToolAdvertisement> {
        Vec::new()
    }
...
impl Agent {
    ...
    pub fn with_action_broker(
        mut self,
        broker: Arc<dyn lokai_domain::sinks::ActionBroker>,
    ) -> Self {
    ...
    pub fn with_process_broker(
        mut self,
        broker: Arc<dyn lokai_domain::sinks::ProcessBroker>,
    ) -> Self {
    ...
    pub fn bind_work_scope(
        &mut self,
        scope: lokai_domain::work_scope::WorkScope,
    ) -> Result<(), AgentError> {
    ...
    async fn execute_gated(&self, name: &str, args: &Value, call_id: &str) -> ToolOutcome {
        ...
        if !broker_handles_approval && self.tools.requires_user_approval(name) {
            if let Some(hook) = &self.approval {
                ...
                lokai_telemetry::fault::inject_fault("during_approval_wait");
                if !hook(req).await {
    ...
    async fn run_tool_execute(
        ...
        let _tool_stage = lokai_telemetry::enter_stage_child("tool");
    ...
    async fn issue_sink_capability(
        ...
        let params = finalize_parameters(
            &kind,
            lokai_domain::execution::CanonicalActionParameters {
                digest: String::new(),
                ...
                process_class: if is_shell {
                    Some(lokai_domain::execution::ProcessClass::ModelRequestedShell)
                } else {
                    None
                },
                sandbox_profile: None,
                expected_output_limits: None,
                schema_version: lokai_domain::CANONICAL_SCHEMA_VERSION,
                tool_arguments: Some(args.clone()),
            },
        );
        let relevant: Vec<lokai_domain::WorkspacePath> = path
            .as_ref()
            .map(|p| lokai_domain::WorkspacePath::new(p.clone()))
            .into_iter()
            .collect();
        ...
        let action = prepare_proposed_action(ProposedAction {
            ...
            session_id: lokai_domain::ids::SessionId::new(
            ...
            run_id: self
                ...
                .run_id
                .as_deref()
                .map(lokai_domain::ids::RunId::new),
            task_id: self
                .config
                .task_id
                .as_deref()
                .map(lokai_domain::ids::TaskId::new),
            attempt_id: self
                .config
                .attempt_id
                .as_deref()
                .map(lokai_domain::ids::AttemptId::new),
            agent_id: Some(lokai_domain::ids::AgentId::new(&self.config.agent_id)),
            workspace_version,
        ...
        F: FnMut(Step) + Send,
    {
        ...
        if let Some(parent_ctx) = lokai_telemetry::extract_context() {
            ...
            lokai_telemetry::inject_context(child);
        } else if let (Some(sid), Some(rid)) = (&self.config.session_id, &self.config.run_id) {
            lokai_telemetry::inject_turn_context(sid, rid, self.config.task_id.as_deref());
        }
        let fabric_trace = lokai_telemetry::extract_context()
            .map(|trace| lokai_domain::TraceContext {
                ...
                run_id: self
                    ...
                    .map(lokai_domain::RunId::new),
                task_id: self
                    .config
                    .task_id
                    .clone()
                    .or(trace.task_id.clone())
                    .map(lokai_domain::TaskId::new),
                attempt_id: self
                    .config
                    .attempt_id
                    .clone()
                    .or(trace.attempt_id.clone())
                    .map(lokai_domain::AttemptId::new),
                scheduler_decision_id: trace.scheduler_decision_id.clone(),
            })
            .unwrap_or_else(|| lokai_domain::TraceContext {
                run_id: self.config.run_id.clone().map(lokai_domain::RunId::new),
                task_id: self.config.task_id.clone().map(lokai_domain::TaskId::new),
                attempt_id: self
                    .config
                    .attempt_id
                    .clone()
        ...
        if convo.messages.is_empty() {
            ...
            if let Some(compiler) = &self.context_compiler {
                ...
                let req = ContextCompileRequest {
                    ...
                    run_id: lokai_domain::ids::RunId::new(
                        self.config
                            .run_id
                            .clone()
                            .unwrap_or_else(|| "local_run".into()),
                    ),
                    task_id: lokai_domain::ids::TaskId::new(
                        self.config
                            .task_id
                            .clone()
        ...
        lokai_telemetry::fault::inject_fault("before_model_request_persistence");
        ...
        'steps: for _ in 0..max_steps {
            ...
            let context_timer =
                lokai_telemetry::StageTimer::start(lokai_telemetry::PerfStage::Context);
            ...
            let _infer_stage = lokai_telemetry::enter_stage_child("infer");
            let resp = {
                ...
                let inference_timer =
                    lokai_telemetry::StageTimer::start(lokai_telemetry::PerfStage::Inference);
            ...
            lokai_telemetry::fault::inject_fault("after_model_request");
...
Fix the JSON to match the tool schema and call the tool again."
                        ...
                        let expansion = match current_fp {
                            Ok(fp) => match (
                                ...
                                (Some(session), Some(run), Some(task)) => {
                                    compiler
                                        .expand(&lokai_domain::ContextExpansionRequest {
                                            handle_id: handle_id.to_string(),
                                            session_id: lokai_domain::SessionId::new(
                                                session.clone(),
                                            ),
                                            run_id: lokai_domain::RunId::new(run.clone()),
                                            task_id: lokai_domain::TaskId::new(task.clone()),
                ...
                lokai_telemetry::fault::inject_fault("before_tool_execution");
                let tool_timer =
                    lokai_telemetry::StageTimer::start(lokai_telemetry::PerfStage::Tool);
                let outcome = self.execute_gated(&name, &args, &call_id).await;
                tool_timer.finish(outcome.ok);
                lokai_telemetry::fault::inject_fault("after_tool_execution");
...
mod tests {
    ...
    use lokai_domain::{ActionKind, CapabilityError, ToolAdvertisement, ToolProposal};
    ...
    fn wire_test_capability_helpers(agent: Agent) -> Agent {
        agent
            ...
            .with_resolve_under_root(Arc::new(|root, rel| {
                let abs =
                    lokai_transaction::fs_ops::resolve_under_root(root, rel).map_err(|_| ())?;
    ...
    #[async_trait]
    impl lokai_domain::sinks::ActionBroker for IssueAllBroker {
        async fn evaluate_and_issue(
            &self,
            action: &ProposedAction,
        ) -> Result<lokai_domain::IssuedCapability, CapabilityError> {
            let now = std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap_or_default()
                .as_secs();
            Ok(lokai_domain::IssuedCapability {
                capability_id: lokai_domain::CapabilityId::new("cap_m2_verify"),
