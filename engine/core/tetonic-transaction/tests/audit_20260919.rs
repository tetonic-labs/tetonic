...
fn missing_rollback_backup_keeps_operation_incomplete_and_retryable() {
    ...
    assert!(tetonic_transaction::commit::rollback_journal(root, &mut journal, &lokai).is_err());
    ...
    tetonic_transaction::commit::rollback_journal(root, &mut journal, &lokai).unwrap();
    assert!(journal.operations[0].rolled_back);
    assert_eq!(
        std::fs::read_to_string(root.join("file.txt")).unwrap(),
        "before"
...
fn unpersisted_completion_is_reconciled_and_rolled_back() {
    ...
    assert!(tetonic_transaction::recovery::resume_recovery(root, &id.0, false).is_err());
}
