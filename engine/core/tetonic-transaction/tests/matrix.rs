...
use tetonic_domain::{DataClass, PatchApproval, TransactionState, WorkspaceVersionScheme};
use tetonic_transaction::{
...
fn commit_txn(
    ...
    stage: impl FnOnce(&mut tetonic_transaction::service::WorkspaceTransaction),
) -> tetonic_domain::CommitResult {
...
fn concurrent_commits_blocked() {
    ...
    let id1 = tetonic_domain::TransactionId::new("txn_a");
    let id2 = tetonic_domain::TransactionId::new("txn_b");
...
fn concurrent_commits_cross_process() {
    if std::env::var("LOKAI_CROSS_LOCK_DIR").is_ok() {
        ...
        let id = tetonic_domain::TransactionId::new("child_txn");
    ...
    let id = tetonic_domain::TransactionId::new("parent_txn");
...
fn recovery_required_blocks_new_transactions() {
    ...
    let lokai = root.join(".lokai");
    std::fs::create_dir_all(lokai.join("journals")).unwrap();
    let txn_id = tetonic_domain::TransactionId::new("blocked_txn");
    let journal = CommitJournal {
        transaction_id: txn_id.clone(),
        base_version: tetonic_domain::WorkspaceVersion {
            repository_id: tetonic_domain::RepositoryId::new("manifest:test"),
            version_scheme: WorkspaceVersionScheme::Manifest,
            git_head: None,
            dirty_state_digest: tetonic_transaction::digest_string("dirty"),
            tracked_state_digest: tetonic_transaction::digest_string("tracked"),
            relevant_path_digests: Default::default(),
            index_generation: None,
        },
        patch_digest: tetonic_transaction::digest_string("patch"),
...
fn non_git_version_is_stable_across_seconds() {
    use tetonic_transaction::version::capture_workspace_version;
...
fn verification_unexpected_source_mutation() {
    ...
    let snap = snapshot_read_digests(
        &root,
        &[
            tetonic_domain::WorkspacePath::new("src.txt"),
            tetonic_domain::WorkspacePath::new("other.txt"),
        ],
    )
    .unwrap();
...
fn cross_process_commit_errors_clearly() {
    if std::env::var("LOKAI_CROSS_COMMIT_DIR").is_ok() {
        ...
        let lokai = PathBuf::from(dir).join(".lokai");
        let id = tetonic_domain::TransactionId::new("child_hold");
        let _lock = WriterLock::acquire(&lokai, &id, "child-proc").unwrap();
        thread::sleep(Duration::from_millis(1500));
        return;
    }
