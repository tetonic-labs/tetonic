...
use lokai_domain::{ActionKind, AuthorizedAction, ExecutionId, ExecutionOutcome};
...
fn redact_process_io(text: &str) -> String {
    lokai_secrets::redact_text_sync_lossy(lokai_secrets::shared_scanner(), text)
}
...
pub struct ProcessExecutor {
    ...
    consumer: Option<Arc<dyn lokai_domain::CapabilityConsumer>>,
...
impl ProcessExecutor {
    ...
    fn log_sandbox_capabilities(&self) {
        ...
        tracing::info!(
            "lokai_sandbox: capabilities platform={} report={}",
            caps.platform,
            serde_json::json!({
                "capabilities": caps,
    ...
    fn validate_capability(
        &self,
        auth: &lokai_domain::execution::AuthorizedAction,
    ) -> Result<(), lokai_domain::sinks::ProcessBrokerError> {
        ...
        let cwd_str = auth
            ...
            .unwrap_or("");
        if cwd_str.is_empty() {
            return Err(lokai_domain::sinks::ProcessBrokerError::Capability(
                lokai_domain::CapabilityError::ScopeMismatch,
            ));
        }
        ...
        self.validator
            ...
            .map_err(lokai_domain::sinks::ProcessBrokerError::Capability)?;
        ...
        if let Some(consumer) = &self.consumer {
            ...
            let now = std::time::SystemTime::now()
                ...
                .unwrap_or_default()
                .as_secs();
            lokai_domain::validate_authorized_action(auth, now)
                .map_err(lokai_domain::sinks::ProcessBrokerError::Capability)
        }
    }
    ...
    pub fn run_verify_with_cancel(
        ...
        let signal = cancel
            .map(|flag| lokai_domain::work_scope::CancellationSignal::from_flag(flag.clone()));
    ...
    pub fn note_lsp_service_start(&self, program: &str) -> Result<(), String> {
        ...
        if self.uses_os_sandbox() {
            ...
            let caps = self.sandbox.capabilities();
            tracing::info!(
                "lokai_sandbox: lsp_service_start execution_id={} program={} mode={:?} caps={} audit={}",
                execution_id,
                program,
    ...
    pub fn sandbox_run_direct(
        ...
        executable: &str,
        arguments: &[String],
        process_class: lokai_domain::execution::ProcessClass,
    ...
    pub fn run_process_sync_cancellable(
        ...
        let result = match &authorized.action.kind {
            ActionKind::ExecuteProcess => {
                let cwd = self.effective_cwd(authorized);
                if let Some(lokai_domain::execution::ProcessClass::BuildVerification) =
