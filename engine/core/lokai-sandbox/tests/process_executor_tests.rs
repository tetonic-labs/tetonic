...
use lokai_sandbox::{EnforcementLevel, NonCodingProcessValidator, ProcessExecutor};
...
#[test]
fn output_is_capped() {
    let huge = "x".repeat(lokai_sandbox::exec::MAX_OUTPUT_BYTES + 1000);
    let capped = lokai_sandbox::exec::truncate_output(&huge);
...
fn cancel_stops_long_run() {
    ...
    #[cfg(windows)]
    let mut cmd = lokai_sandbox::process_executor::shell_command("ping -n 30 127.0.0.1 > nul");
    #[cfg(not(windows))]
    let mut cmd = lokai_sandbox::process_executor::shell_command("sleep 30");
    cmd.current_dir(&dir);
    let handle = std::thread::spawn(move || {
        lokai_sandbox::exec::command_output_with_timeout_and_cancel(
            &mut cmd,
            Duration::from_secs(60),
            Some(&cancel_flag),
            lokai_sandbox::exec::EnvMode::Minimal,
        )
    });
...
async fn process_claimed_version_without_validator_denied() {
    ...
    let fake_wv = lokai_domain::WorkspaceVersion {
        repository_id: lokai_domain::RepositoryId::new("repo"),
        version_scheme: lokai_domain::WorkspaceVersionScheme::Git,
        git_head: None,
        dirty_state_digest: lokai_domain::ContentDigest::new("d1"),
        tracked_state_digest: lokai_domain::ContentDigest::new("d2"),
