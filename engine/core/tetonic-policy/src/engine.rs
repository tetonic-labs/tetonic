...
use tetonic_domain::ProjectPlacementPolicy;
use tetonic_domain::{
...
impl PolicyEngine {
    ...
    fn check_disclosure_tier(&self, job: &FabricJobDraft) -> PolicyDecision {
        ...
        match job.disclosure_tier {
            tetonic_domain::DisclosureTier::MetadataOnly => PolicyDecision::Deny {
        ...
        let decision = match &action.kind {
            ActionKind::ExecuteProcess => {
                ...
                if action.parameters.process_class
                    == Some(tetonic_domain::execution::ProcessClass::BuildVerification)
                    && !self.verify_allowed()
                {
...
mod tests {
    ...
    fn evaluate_action_denies_verify_when_disabled() {
        ...
        let action = ProposedAction {
            ...
            session_id: tetonic_domain::ids::SessionId::new("s"),
            run_id: None,
            task_id: None,
            attempt_id: None,
            agent_id: Some(tetonic_domain::ids::AgentId::new("root")),
