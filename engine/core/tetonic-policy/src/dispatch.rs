...
impl DispatchGuard for PolicyDispatchGuard {
    fn project_placement_policy(&self) -> tetonic_domain::ProjectPlacementPolicy {
    ...
    fn evaluate(&self, request: &DispatchRequest) -> Result<DispatchDecision, DispatchDenied> {
        ...
        let disclosure = tetonic_domain::DisclosureTier::Auditable;
        ...
        if !decision.allowed() {
            let reason = match &decision {
                tetonic_domain::PolicyDecision::Deny { reason } => reason.as_str(),
...
pub fn classification_from_session_data_class(class: DataClass) -> tetonic_domain::Classification {
...
mod adversarial_tests {
    ...
    use super::*;
    use tetonic_domain::{Classification, ClassificationSource, DataClass, ProjectPlacementPolicy};
    ...
    fn external_untrusted_blocks_repository_source() {
        let req = DispatchRequest {
            ...
            post_redaction: false,
            worker_trust: Some(tetonic_domain::WorkerTrust::ExternalUntrusted),
            project_policy: tetonic_domain::ProjectPlacementPolicy::default(),
        };
