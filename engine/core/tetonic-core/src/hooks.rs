...
/// [`tetonic_domain::ToolHost::requires_user_approval`] reports true are routed
...
/// take Session. Composition supplies `tetonic_transaction::version::capture_workspace_version`.
pub type CaptureWorkspaceVersion = Arc<
    dyn Fn(
            &Path,
            &[tetonic_domain::WorkspacePath],
        ) -> Result<tetonic_domain::WorkspaceVersion, String>
        + Send
        + Sync,
>;
...
pub type SpawnHook = Box<
    dyn for<'a> Fn(
            ...
            -> Pin<Box<dyn Future<Output = tetonic_domain::ToolOutcome> + Send + 'a>>
        + Send
        + Sync,
>;
