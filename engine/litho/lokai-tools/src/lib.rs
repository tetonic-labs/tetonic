...
thread_local! {
    static INDEX_CACHE: RefCell<HashMap<PathBuf, Rc<dyn lokai_domain::CodeIndex>>> =
        RefCell::new(HashMap::new());
    static MEMORY_CACHE: RefCell<HashMap<PathBuf, Rc<lokai_memory::Store>>> =
        RefCell::new(HashMap::new());
}
...
pub struct Tools {
    ...
    code_index_open: Option<Arc<dyn lokai_domain::CodeIndexOpen>>,
    ...
    lsp_open: Option<Arc<dyn lokai_domain::LspSessionOpen>>,
    ...
    capability_consumer: Option<Arc<dyn lokai_domain::CapabilityConsumer>>,
}
...
impl Tools {
    pub fn new(ws: Workspace, allow_shell: bool) -> Self {
        ...
        let txn = Arc::new(
            lokai_transaction::WorkspaceTransactionService::new(
                &root,
                lokai_transaction::WorkspaceTxnConfig::default(),
            )
    ...
    pub fn with_capability_consumer(
        mut self,
        consumer: Arc<dyn lokai_domain::CapabilityConsumer>,
    ) -> Self {
        self.reset_lsp_binding();
    ...
    pub fn commit_staged(&self) -> Result<lokai_domain::CommitResult, ToolError> {
        self.mutation
            .commit_staged(lokai_domain::DataClass::RepositorySource)
    }
    ...
    pub fn with_code_index_open(mut self, opener: Arc<dyn lokai_domain::CodeIndexOpen>) -> Self {
    ...
    pub fn execute_authorized(
        ...
        name: &str,
        args: &Value,
        auth: Option<&lokai_domain::AuthorizedAction>,
    ) -> ToolOutcome {
    ...
    fn open_index(&self) -> Result<(Rc<dyn lokai_domain::CodeIndex>, String), ToolError> {
        let path = self
            .index_db
            .as_ref()
        ...
        let idx = INDEX_CACHE.with(|c| -> Result<Rc<dyn lokai_domain::CodeIndex>, ToolError> {
            let mut map = c.borrow_mut();
            if let Some(i) = map.get(path) {
                return Ok(i.clone());
            }
            let opened: Rc<dyn lokai_domain::CodeIndex> = Rc::from(
                opener
                    .open(path)
    ...
    fn open_memory(&self) -> Result<Rc<lokai_memory::Store>, ToolError> {
        let path = self
            .memory_db
            .as_ref()
            .ok_or_else(|| ToolError::Other("audit memory not enabled for this session".into()))?;
        MEMORY_CACHE.with(|c| -> Result<Rc<lokai_memory::Store>, ToolError> {
            let mut map = c.borrow_mut();
            ...
            let opened = Rc::new(
                ...
                lokai_memory::Store::open_readonly(path)
        ...
        ignore::WalkBuilder::new(&base)
            ...
            .run(|| {
                ...
                Box::new(move |result| {
                    ...
                    if lokai_transaction::fs_ops::is_symlink_or_reparse(entry.path()) {
                        return ignore::WalkState::Continue;
                    }
    ...
    pub fn apply_verify_overlay(
        &self,
        authorized: &mut lokai_domain::AuthorizedAction,
    ...
    pub fn run_verify_sink(&self, authorized: &lokai_domain::AuthorizedAction) -> (bool, String) {
        let overlay = self.verification_overlay_if_staged().ok().flatten();
