...
impl Workspace {
    ...
    pub fn resolve(&self, rel: &str) -> Result<PathBuf, ToolError> {
        lokai_transaction::fs_ops::resolve_under_root(&self.root, rel).map_err(map_txn)
    }
...
fn map_txn(err: lokai_transaction::TransactionError) -> ToolError {
    match err {
        lokai_transaction::TransactionError::OutsideWorkspace(p) => ToolError::OutsideWorkspace(p),
        lokai_transaction::TransactionError::Io(s) => ToolError::Io(s),
...
pub fn read_to_string_nofollow(path: &Path) -> Result<String, ToolError> {
    lokai_transaction::fs_ops::read_to_string_nofollow(path).map_err(map_txn)
}

pub fn write_bytes_nofollow(path: &Path, content: &[u8]) -> Result<(), ToolError> {
    lokai_transaction::fs_ops::write_bytes_nofollow(path, content).map_err(map_txn)
}

pub fn create_dir_all_nofollow(root: &Path, dir: &Path) -> Result<(), ToolError> {
    lokai_transaction::fs_ops::create_dir_all_nofollow(root, dir).map_err(map_txn)
}

pub fn remove_nofollow(path: &Path) -> Result<(), ToolError> {
    lokai_transaction::fs_ops::remove_nofollow(path).map_err(map_txn)
}
