...
pub trait CapacityService: Send + Sync {
    ...
    ) -> Result<Vec<tetonic_capacity::ProfileSummary>, AppError>;
    fn activate_profile(
        &self,
    ...
    ) -> Result<tetonic_capacity::RuntimeProfile, AppError>;
    fn import_profile(
        &self,
    ...
    fn snapshot_admission_status(&self, node_id: &str) -> Option<tetonic_capacity::CapacityStatus>;
}
...
impl CapacityService for DefaultCapacityService {
    fn begin_optimize(
        ...
        let job_id = tetonic_memory::new_id("capjob");
        emit(
            &self.events,
        ...
        Ok(BeginOptimizeResultPayload {
            job_id,
            depth: tetonic_capacity::OptimizeDepth::parse(&cmd.depth),
    ...
    async fn get_capacity_status(
        ...
        let (status, _) = tetonic_capacity::capacity_status_for_store(
            store,
            &cmd.node_id,
            &cmd.client,
            cmd.ollama_version,
    ...
    async fn get_capacity_doctor(
        ...
        let (status, diagnosis) = tetonic_capacity::capacity_status_for_store(
            store,
            &cmd.node_id,
            &cmd.client,
            cmd.ollama_version,
        ...
        let defaults = store
            .write_sync({
                ...
                move |db| {
                    ...
                    Ok::<_, AppError>(tetonic_capacity::load_inference_defaults(db, &node_id))
                }
            })
    ...
    fn rollback_profile(
        ...
        let (prev, defaults) = store
            .write_sync({
                let node_id = cmd.node_id.clone();
                move |db| {
                    let prev = tetonic_capacity::ProfileStore::new(db)
                        .rollback(&node_id, role)
                        .map_err(|e| AppError::InvalidRequest(format!("rollback: {e}")))?;
                    let defaults = tetonic_capacity::load_inference_defaults(db, &node_id);
    ...
    fn import_profile(
        ...
        let mut profile: tetonic_capacity::RuntimeProfile = serde_json::from_str(&cmd.json)
        ...
        if cmd.refresh_fingerprint {
            profile.hardware =
                tetonic_capacity::detect_hardware(profile.hardware.ollama_version.clone());
        }
        ...
        store
            .write_sync({
                ...
                move |db| {
                    let ps = tetonic_capacity::ProfileStore::new(db);
                    ps.append(&profile)
                        .map_err(|e| AppError::PersistenceFailed(format!("store profile: {e}")))?;
                    if activate {
                        ps.activate(tetonic_capacity::LOCAL_NODE_ID, profile.role, &profile.id)
                            .map_err(|e| AppError::InvalidRequest(format!("activate: {e}")))?;
                    }
                    Ok::<_, AppError>(())
                }
    ...
    async fn run_optimize(
        ...
        let opts = tetonic_capacity::OptimizeOptions {
            depth: begin.depth,
        ...
        let outcome = match tetonic_capacity::run_optimize(
            cmd.client,
            store,
            &job_id,
            opts,
        ...
        {
            Ok(o) => o,
            Err(e) => tetonic_capacity::OptimizeOutcome {
                job_id: job_id.clone(),
                state: if matches!(e, tetonic_capacity::OptimizeError::Cancelled) {
                    tetonic_capacity::JobState::Cancelled
                } else {
                    tetonic_capacity::JobState::Failed
                },
                profile_ids: vec![],
        ...
        let state_str = match outcome.state {
            tetonic_capacity::JobState::Succeeded => "succeeded",
            tetonic_capacity::JobState::Cancelled => "cancelled",
            _ => "failed",
        };
    ...
    fn snapshot_admission_status(&self, node_id: &str) -> Option<tetonic_capacity::CapacityStatus> {
        ...
        store
            .read_sync({
                ...
                move |db| Some(tetonic_capacity::admission_status_from_store(db, &node_id))
            })
            .ok()
            .flatten()
    }
