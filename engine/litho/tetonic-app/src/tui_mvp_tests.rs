...
async fn tui_mvp_read_tool_and_followup_through_compute_broker() {
    for mode in ["off", "auto"] {
        ...
        let plane = crate::build_compute_plane(ComputePlaneRequest {
            guard,
            ollama_base: url,
            policy: Arc::new(tetonic_policy::PolicyEngine::default()),
...
impl tetonic_inference::InferenceProvider for PanicProvider {
    async fn chat(
        &self,
...
#[async_trait::async_trait]
impl tetonic_inference::InferenceProvider for WaitingProvider {
    async fn chat(
        &self,
        _: tetonic_inference::ChatRequest,
        _: &mut tetonic_inference::TokenSink<'_>,
