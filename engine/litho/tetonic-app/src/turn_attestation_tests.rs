...
use crate::*;
use std::sync::Arc;
use tetonic_artifact::{verify_stored_content, LocalArtifactStore};
use tetonic_domain::artifact::{ArtifactLocation, ArtifactStore};
use tetonic_domain::ids::ArtifactId;
use tetonic_domain::{AttemptState, CandidateOutcome, CompletionKind, RunState};
...
fn make_app_with_store(
    ...
    let runtime = tetonic_runtime::EngineRuntime::new(policy.clone(), None, artifacts.clone());
...
async fn start_and_plan(
    app: &Application,
) -> (String, tetonic_domain::RunId, tetonic_domain::AttemptId) {
...
async fn accepted_digest(app: &Application, run_id: tetonic_domain::RunId) -> (String, String) {
    let snap = app
        .runs
        ...
        .await
        .expect("snapshot");
    let task_id = tetonic_domain::TaskId::new(format!("task_root_{run_id}"));
    let art = snap
        .tasks
        .get(&task_id)
...
async fn finish_turn_seals_matching_artifact() {
    let dir = std::env::temp_dir().join(tetonic_memory::new_id("h32"));
...
async fn accept_path_marks_accepted_and_provenance() {
    use tetonic_domain::artifact::{ArtifactState, ProvenanceTrustLabel};
    let dir = std::env::temp_dir().join(tetonic_memory::new_id("r24"));
    let (app, artifacts, mem) = make_app_with_store(&dir);
    let (session_id, run_id, attempt_id) = start_and_plan(&app).await;
    {
        let _ = mem.write_sync({
    ...
    assert!(matches!(
        missing,
        tetonic_domain::artifact::ArtifactError::NotFound(_)
    ));
}
...
async fn tampered_artifact_fails_verify() {
    ...
    assert!(
        matches!(
            err,
            tetonic_domain::artifact::ArtifactError::DigestMismatch { .. }
        ),
        "{err:?}"
    );
}
...
async fn failed_turn_records_error_digest_not_success() {
    ...
    let err_task = tetonic_domain::TaskId::new(format!("task_root_{err_run}"));
    assert!(
        err_snap
            .tasks
            .get(&err_task)
