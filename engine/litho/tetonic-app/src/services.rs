    ...
    async fn start_session(
        ...
        let (session_id, resumed, messages_loaded, messages, resume_state) = if resume {
                    ...
                    let resume_state = match prior_status.as_deref() {
                        Some("running") => {
                            let recovery = db
                                ...
                                .map_err(|e| AppError::PersistenceFailed(format!("audit: {e}")))?
                                .and_then(|op| tetonic_core::TurnState::from_str(&op.state))
            ...
            let sid = match &self.store {
                ...
                None => Ok(tetonic_memory::new_id("sess")),
            }
            .map_err(|e| AppError::PersistenceFailed(format!("audit: {e}")))?;
        ...
        let orchestration_mode = cmd
            .orchestration
            .as_deref()
            .map(tetonic_orchestrator::OrchestrationMode::parse)
            .unwrap_or(tetonic_orchestrator::OrchestrationMode::Single);
        let critic_enabled = cmd
            .critic
            .unwrap_or(orchestration_mode == tetonic_orchestrator::OrchestrationMode::Auto);
        ...
        let tool_workspace = tetonic_tools::ensure_session_worktree(
        ...
        let conversation = if resumed {
            tetonic_core::Conversation::from_audit_messages(messages.clone())
        } else {
        ...
        let data_class = tetonic_policy::data_class_name(plan.data_class).to_string();

        // R02: bind live session_id at session start (CLI + daemon share this path).
        tetonic_telemetry::inject_session_context(&session_id);
...
impl SessionLifecycle for DefaultSessionService {
    fn end_session(&self, cmd: EndSessionCommand) -> Result<(), AppError> {
        ...
        tetonic_tools::remove_session_worktree(
            std::path::Path::new(&cmd.workspace_root),
            &cmd.session_id,
        );
