...
use tetonic_inference::hosted::registry::{build_hosted_config, find_provider};
...
pub(crate) struct RegisteredProfile {
    pub provider: Arc<dyn tetonic_inference::InferenceProvider>,
    pub broker: Option<Arc<tetonic_broker::DefaultComputeBroker>>,
...
impl Application {
    ...
    pub fn model_catalog(&self, session_id: &str) -> Result<ModelCatalog, AppError> {
        ...
        for p in tetonic_inference::hosted::registry::PROVIDERS {
    ...
    pub fn register_cloud_provider_model(
        ...
        profiles.insert(
            ...
            RegisteredProfile {
                provider: provider as Arc<dyn tetonic_inference::InferenceProvider>,
                broker,
                models,
                num_ctx,
            },
    ...
    pub fn register_inference_profile(
        ...
        profiles.insert(
            name.into(),
            RegisteredProfile {
                provider: plane.provider.clone() as Arc<dyn tetonic_inference::InferenceProvider>,
...
mod tests {
    ...
    async fn registered_profile_replaces_provider_broker_and_budget_together() {
        ...
        let (sink, _) = crate::events::RecordingEventSink::new();
        let app = Application::bootstrap_mock(dir.path(), sink, vec![]);
        let pooled = Arc::new(tetonic_inference::PooledProvider::new(
            Arc::new(tetonic_inference::OllamaProvider::new(
        ...
        app.select_session_model(&session.session_id, &choice.id, catalog.revision)
            .unwrap();
        let host = app.build_session_host(&session.session_id).unwrap();
        let expected_provider: Arc<dyn tetonic_inference::InferenceProvider> = provider;
