...
use tetonic_domain::{AttemptId, EventType, RunState};
...
fn get_session_run_id(live: &tetonic_app::session_live::LiveSession) -> tetonic_domain::RunId {
    live.last_run_id()
...
async fn work04_cancel_run_stops_inflight_executor() {
    ...
    local
        .run_until(async {
            ...
            let finish = rx.await.unwrap_or(tetonic_app::commands::TurnFinish {
                ok: false,
                canceled: true,
                error: None,
            });
...
async fn work04_lease_loss_or_heartbeat_failure_fails_closed() {
    ...
    let comp_err = app
        .runs
        .complete_turn(
            &tetonic_app::commands::CompleteTurnCommand {
                session_id: started.session_id.clone(),
