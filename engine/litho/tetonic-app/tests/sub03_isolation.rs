...
fn sub03_lokai_core_production_toml_has_no_tools_or_transaction() {
    let toml = read(engine_root().join("core/tetonic-core/Cargo.toml"));
    let deps = cargo_section(&toml, "dependencies");
    let dev = cargo_section(&toml, "dev-dependencies");
    assert!(
        !lists_crate(&deps, "tetonic-tools") && !lists_crate(&deps, "lokai-tools"),
        "[dependencies] must not list lokai-tools: {deps}"
    );
    assert!(
        !lists_crate(&deps, "tetonic-transaction") && !lists_crate(&deps, "lokai-transaction"),
    ...
    assert!(
        lists_crate(&dev, "tetonic-tools")
            || lists_crate(&dev, "tetonic-transaction")
            || lists_crate(&dev, "lokai-tools")
            || lists_crate(&dev, "lokai-transaction"),
...
fn sub03_production_agent_rs_has_no_tools_or_transaction_imports() {
    ...
    assert!(
        !agent.contains("tetonic_tools::") && !agent.contains("use tetonic_tools"),
        "production agent.rs must not import tetonic_tools"
    );
    assert!(
        !agent.contains("tetonic_transaction::") && !agent.contains("use tetonic_transaction"),
        "production agent.rs must not import tetonic_transaction"
    );
}
