...
fn engine_crate(crate_name: &str) -> PathBuf {
    ...
    for layer in layers {
        ...
        let alt = engine_root
            .join(layer)
            .join(crate_name.replace("lokai-", "tetonic-"));
        if alt.exists() {
            return alt;
        }
    }
...
fn sub02_compile_invocation_does_not_use_toolhost_catalog() {
    let def_src = production_src("tetonic-app", "definition.rs");
    assert!(
...
fn explain_inv() -> AgentInvocation {
    AgentInvocation {
        ...
        discipline: tetonic_app::definition::coding_loop_discipline(true, false),
    }
}
