...
use tetonic_core::{Agent, AgentConfig, Conversation, SpawnRequest, Step};
use tetonic_domain::ids::{AttemptId, ReservationId, RunId, TaskId};
use tetonic_domain::{AgentInvocation, CandidateOutcome};
...
#[async_trait::async_trait]
impl tetonic_inference::InferenceProvider for FinishOnceProvider {
    async fn chat(
        &self,
        ...
        if !*done {
            *done = true;
            return Ok(tetonic_inference::ChatResponse {
        ...
        Ok(tetonic_inference::ChatResponse {
            message: tetonic_inference::Message::assistant("done"),
            usage: Default::default(),
            provenance: Default::default(),
        })
    }
