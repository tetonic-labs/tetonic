...
fn code03_child_task_is_ready_not_blocked() {
    ...
    let dag = crate_src("../../mantle/lokai-run/src/dag.rs");
...
fn code03_infer_hop_addtask_untouched() {
    let src = crate_src("../../mantle/lokai-run/src/infer_admission.rs");
...
fn code03_turn_rs_has_no_leftover_turn() {
    let src = crate_src("../../mantle/lokai-orchestrator/src/turn.rs");
...
fn code03_spawn_host_has_no_refcell() {
    let src = crate_src("../../mantle/lokai-orchestrator/src/spawn_host.rs");
    let prod = production_prefix(&src);
...
fn code03_llm_route_has_no_fabric_none() {
    let src = crate_src("../../mantle/lokai-orchestrator/src/router_llm.rs");
...
fn code03_chat_request_fallback_untouched() {
    let src = crate_src("../../mantle/lokai-broker/src/chat_request.rs");
...
fn code03_kernel_spawn_agent_name_check_remains() {
    let src = crate_src("../../mantle/lokai-orchestrator/src/spawn.rs");
    assert!(src.contains("pub fn spawn_agent_tool_name"));
}

#[test]
fn code03_orchestrator_cargo_still_lists_tools() {
    let src = crate_src("../../mantle/lokai-orchestrator/Cargo.toml");
    assert!(src.contains("lokai-tools"));
    assert!(src.contains("lokai-core"));
}
