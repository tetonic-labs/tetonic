...
use lokai_app::commands::{CompleteTurnCommand, RunTurnCommand};
use lokai_app::events::ApplicationEventSink;
use lokai_app::services::{
...
use lokai_run::{command_envelope, DurableRunSupervisor, RunSupervisor};
...
struct TestHarness {
    ...
    store: lokai_memory::SharedStore,
}
...
fn create_harness() -> TestHarness {
    ...
    let live = Arc::new(lokai_app::SessionLiveStore::new());
    let runs = Arc::new(
        ...
        .with_identity_supplier(Arc::new(|input| {
            lokai_app::definition::coding_identity_and_job_spec(input)
        })),
    );
    TestHarness {
        runs,
...
async fn start_test_session(store: &lokai_memory::SharedStore) -> String {
    store
        .write(|db| {
            db.start_session("test_workspace", "agent", "test_model")
                .map_err(|e| lokai_app::errors::AppError::PersistenceFailed(e.to_string()))
        })
        .await
        .expect("write")
...
async fn fin03_competing_finalizers_fail_closed() {
    ...
    harness
        .supervisor
        .handle(RunCommand::CreateAttempt(lokai_domain::CreateAttempt {
...
/// 5. Manager modules have zero references to CodingAgentDefinition, commit_staged_if_any, explain_turn, or lokai_tools::.
#[test]
fn fin03_manager_modules_have_no_coding_definition_or_commit_staged() {
    ...
    for rel in files {
        ...
        assert!(
            ...
            "production {rel} must not reference lokai_tools::"
        );
    }
}
...
async fn fin03_cancellation_during_verify_blocks_workspace_commit() {
    ...
    local
        .run_until(async move {
            ...
            let cancel = harness
                .runs
                .cancel_run(lokai_app::commands::CancelByRunCommand {
