...
use lokai_domain::{AttemptId, AttemptState, CandidateOutcome, RunId, WorkerId};
use lokai_inference::*;
...
impl FabricNodeProvider for Worker {
    ...
    fn fabric_capabilities(&self) -> lokai_fabric_protocol::WorkerCapabilityAdvertisement {
        lokai_fabric_protocol::WorkerCapabilityAdvertisement::fabric_v1_full(vec![
            lokai_fabric_protocol::JobKind::Infer,
        ])
    }
...
struct Redactions;
impl lokai_domain::secrets::OutboundRedactionSink for Redactions {
    fn record(&self, _: &lokai_domain::secrets::OutboundRedaction) -> Result<(), String> {
        Ok(())
    }
}
...
async fn managed_loop_survives_real_broker_failover() {
    ...
    let store = lokai_memory::SharedStore::open(tmp.path().join("managed.db"), 1).unwrap();
    let supervisor: Arc<dyn RunSupervisor> =
        Arc::new(lokai_run::DurableRunSupervisor::new(Some(store.clone())));
    ...
    let manager =
        lokai_run::ManagedRunService::new(supervisor.clone(), Some(store), artifacts, policy);
    let calls = Arc::new(AtomicUsize::new(0));
    let hops = Arc::new(Mutex::new(Vec::new()));
    let mut registry = lokai_fabric_protocol::CapabilityRegistry::new();
    ...
    for id in ["proof-worker-a", "proof-worker-b"] {
        ...
        let caps = lokai_fabric_protocol::WorkerCapabilities::legacy_infer_profile(
            worker_id.clone(),
            "boot",
            1,
            0,
    ...
    let pooled = Arc::new(
        PooledProvider::new(local, workers)
            ...
            .with_dispatch_guard(Arc::new(lokai_policy::PolicyDispatchGuard::new(Arc::new(
                lokai_policy::PolicyEngine::default(),
            ))))
            .with_worker_trust_resolver(Arc::new(|_| {
                Some((lokai_domain::WorkerTrust::OwnerControlledEstate, 0))
            })),
    );
    let budgets = Arc::new(lokai_broker::HierarchicalBudgetLedger::new(
        lokai_broker::BudgetLimits::default(),
    ));
    let queue = Arc::new(lokai_broker::QueueManager::new(
        Default::default(),
        Default::default(),
    ));
    let admission = Arc::new(lokai_broker::HierarchicalAdmissionController::new(
        budgets, queue,
    ));
    let broker = Arc::new(lokai_broker::DefaultComputeBroker::new(
        admission,
        Arc::new(lokai_broker::InMemoryReservationStore::default()),
        Some(Arc::new(lokai_broker::InferenceTargetAdapter::new(pooled))),
        Some(supervisor.clone()),
    ));
    broker.set_capability_registry(registry);
    let inference = Arc::new(
        lokai_broker::BrokerInferenceProvider::new(broker).with_outbound_scanner(
            Arc::new(lokai_secrets::ScannerEngine::default_engine()),
            Arc::new(Redactions),
        ),
    );
    ...
    let identity =
        lokai_app::definition::CodingAgentDefinition::production().coding_identity_record();
    let command = lokai_run::StartIdentityJobCommand {
        job_spec: lokai_domain::AgentJobSpec {
            identity_id: identity.id.clone(),
            definition_digest: identity.bound_definition_digest.clone(),
            input_digest: lokai_run::job_input_digest("complete the proof"),
    ...
    assert_eq!(agent_snapshot.state, lokai_domain::RunState::Succeeded);
