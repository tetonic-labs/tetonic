...
use lokai_app::commands::{RunTurnCommand, StartIdentityJobCommand, StartSessionCommand};
use lokai_app::definition::CodingAgentDefinition;
use lokai_app::events::{ApplicationEvent, ApplicationEventSink};
use lokai_app::services::RunService;
...
async fn work05_assertion1_local_executor_mismatched_attempt_fails_before_turn() {
    ...
    let mut executor = lokai_runtime::LocalAgentAttemptExecutor::new(&mut agent, &mut conv, |_| {});
...
async fn work05_assertion2_child_jobs_persist_distinct_spec_and_input_digest() {
    ...
    let db_path = fs::read_dir(tmp.path())
        ...
        .unwrap();
    let store = lokai_memory::SharedStore::open(&db_path, 1).unwrap();
    let recovered = lokai_run::DurableRunSupervisor::new(Some(store));
...
async fn sessionless_cancel_stops_a_pending_provider() {
    ...
    let cancel = async {
        ...
        app.runs
            .cancel_run(lokai_app::commands::CancelByRunCommand { run_id: run })
            .await
            .unwrap();
    };
    ...
    local
        .run_until(async move {
            ...
            let outcome = run_orchestrated_turn(
                ...
                |_aid, _step| {},
                None::<fn(lokai_orchestrator::RouteDecision, &'static str)>,
                TestRootExecute {
