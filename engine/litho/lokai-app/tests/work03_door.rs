...
use lokai_app::definition::{CodingAgentDefinition, CODING_IDENTITY_ID};
use lokai_app::events;
use lokai_app::{Application, ApplicationDependencies};
use lokai_core::{Agent, AgentConfig};
use lokai_domain::{
...
async fn work03_start_identity_job_finishes_through_finish_turn_run() {
    let execution = crate_src("../../mantle/lokai-run/src/managed/execution.rs");
...
fn work03_broker_has_no_session_as_run() {
    let src = crate_src("../../mantle/lokai-broker/src/broker.rs");
    let prod = production_prefix(&src);
    assert!(!prod.contains("RunId::new(session_id)"));
    assert!(prod.contains("fn cancel_run_jobs"));
    let mut hits = Vec::new();
    walk_production_rs(
        &PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../mantle/lokai-broker/src"),
        &mut |path, text| {
...
fn work03_session_cancel_cancels_indexed_hops_by_stored_run_id() {
    let src = crate_src("../../atmos/lokai-inference/src/pooled.rs");
...
fn work03_job_record_stores_run_id() {
    let src = crate_src("../../atmos/lokai-inference/src/attempt.rs");
...
fn work03_agent_context_no_session_run_fallback() {
    let src = crate_src("../../core/lokai-core/src/agent.rs");
    let prod = production_prefix(&src);
