...
fn cap01_neutral_runtime_assembly_no_default_walk() {
    ...
    let ws = lokai_tools::Workspace::new(temp_dir.path()).unwrap();
    let tools = lokai_tools::Tools::new(ws, false);
    ...
    let supplied_compiler: Arc<dyn ContextCompiler> = Arc::new(
        lokai_context::pipeline::ContextCompiler::new(dummy_provider),
    );
...
fn cap01_runtime_crate_clean_of_repository_heuristics() {
    let runtime_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../core/lokai-runtime");
