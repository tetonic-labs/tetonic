...
fn gate01_patch_pipeline_not_in_src() {
    let lib = crate_src("../../atmos/lokai-fabric-client/src/lib.rs");
    assert!(!lib.contains("pub mod patch_pipeline"));
    assert!(!lib.contains("pub use patch_pipeline"));
    let src_dir =
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../atmos/lokai-fabric-client/src");
    assert!(!src_dir.join("patch_pipeline.rs").exists());
    let helper = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../atmos/lokai-fabric-client/tests/common/patch_pipeline.rs");
    assert!(helper.exists());
    let helper_src = fs::read_to_string(&helper).expect("helper");
    assert!(helper_src.contains("lokai_fabric_client::validate_patch_acceptance"));
    assert!(helper_src.contains("lokai_fabric_client::FabricClientError"));
    assert!(!helper_src.contains("crate::result_accept"));
    assert!(!PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../atmos/lokai-fabric-client/tests/patch_pipeline.rs")
        .exists());
}
...
fn gate01_destination_lokai_tools_remain() {
    for rel in ["Cargo.toml", "../../mantle/lokai-orchestrator/Cargo.toml"] {
