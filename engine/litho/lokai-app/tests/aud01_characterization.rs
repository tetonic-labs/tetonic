...
use lokai_artifact::LocalArtifactStore;
use lokai_domain::{AttemptState, RunState};
...
fn make_app_with_store(dir: &std::path::Path) -> (Application, lokai_memory::SharedStore) {
...
async fn aud01_bh_recover_no_future_resume() {
    let dir = std::env::temp_dir().join(lokai_memory::new_id("aud01_rec"));
    let ws = dir.join("ws");
...
async fn aud01_defect_bh_fin_err_current() {
    ...
    let dir = std::env::temp_dir().join(lokai_memory::new_id("aud01_finerr"));
