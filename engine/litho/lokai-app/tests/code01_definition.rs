...
use lokai_app::definition::{CodingAgentDefinition, CodingIdentityPolicy, ContextBinding};
use lokai_orchestrator::{RoleId, SpecialistPack};
...
fn code01_daemon_parse_uses_production() {
    ...
    for alias in ALIASES {
        assert_eq!(
            lokai_app::coding_pack::CodingPack.parse(alias),
            def.parse(alias)
        );
    }
...
fn code01_identity_policy_is_not_session_and_is_not_persisted() {
    ...
    let domain_identity = fs::read_to_string(engine_root.join("core/lokai-domain/src/identity.rs"))
...
fn code01_app001_inventory_not_established() {
    ...
    for (layer, name) in [
        ("substrate", "lokai-core"),
        ("substrate", "lokai-runtime"),
        ("compute", "lokai-fabric-client"),
        ("infrastructure", "lokai-node"),
    ] {
