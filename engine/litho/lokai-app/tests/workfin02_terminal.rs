...
use lokai_run::detect_recovery_required;
...
fn workfin02_fail_hop_without_job_spec_is_not_d() {
    let infer = crate_src("../../mantle/lokai-run/src/infer_admission.rs");
...
fn workfin02_recover_at_startup_is_sole_recovery_required_writer() {
    let recovery = crate_src("../../mantle/lokai-run/src/recovery.rs");
    assert!(!recovery.contains("fn mark_recovery_required"));
    let service = crate_src("../../mantle/lokai-run/src/service.rs");
...
fn workfin02_txn_artifact_binds_task_and_attempt() {
    let txn = crate_src("../../core/lokai-transaction/src/service.rs");
...
fn workfin02_bind_effect_identity_sets_txn_fields() {
    let tools = crate_src("../../litho/lokai-tools/src/lib.rs");
    assert!(tools.contains("pub fn bind_effect_identity"));
    let mutation = crate_src("../../litho/lokai-tools/src/mutation.rs");
...
fn workfin02_journal_persist_fsyncs() {
    let src = crate_src("../../core/lokai-transaction/src/journal.rs");
...
fn workfin02_journal_intent_already_before_first_apply() {
    let src = crate_src("../../core/lokai-transaction/src/service.rs");
...
fn workfin02_result_accept_does_not_build_complete_attempt() {
    let src = crate_src("../../atmos/lokai-fabric-client/src/result_accept.rs");
...
fn workfin02_legacy_result_does_not_apply_complete_attempt() {
    let src = crate_src("../../atmos/lokai-fabric-client/src/legacy_result.rs");
    assert!(!production_prefix(&src).contains("apply_complete_attempt"));
}

#[test]
fn workfin02_bridge_does_not_submit_complete_attempt() {
    let trait_src = crate_src("../../atmos/lokai-fabric-client/src/run_bridge.rs");
...
fn workfin02_no_public_remote_patch_commit() {
    ...
    let fabric = crate_src("../../atmos/lokai-fabric-client/src/legacy_result.rs");
