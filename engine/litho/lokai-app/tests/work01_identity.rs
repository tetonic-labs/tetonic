...
use lokai_run::{binding_input_digest, job_input_digest};
use lokai_runtime::LocalAgentAttemptExecutor;
...
fn make_app() -> (Application, lokai_memory::SharedStore) {
...
async fn inspect(app: &Application, run_id: &RunId) -> lokai_domain::RunSnapshot {
    app.runs
...
fn work01_code01_absence_pin_replaced() {
    ...
    let domain = fs::read_to_string(engine_root.join("core/lokai-domain/src/identity.rs"))
        .expect("identity.rs");
    assert!(domain.contains("struct AgentIdentity"));
    assert!(domain.contains("struct AgentJobSpec"));
    for (layer, name) in [
        ("manager", "lokai-run"),
        ("substrate", "lokai-core"),
        ("infrastructure", "lokai-node"),
    ] {
        assert!(
