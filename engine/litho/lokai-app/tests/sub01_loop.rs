...
use lokai_domain::{CandidateOutcome, CompletionKind};
use lokai_inference::{
    ChatRequest, ChatResponse, FabricSnapshot, InferenceError, InferenceProvider, Message,
    TokenSink,
};
use lokai_orchestrator::SpecialistPack;
...
fn sub01_instructions_compile_from_product() {
    ...
    let overlay = production().overlay(&lokai_orchestrator::RoleId::new("planner"));
    let cfg = AgentConfig {
    ...
    async fn chat(
        ...
        Ok(ChatResponse {
            message: Message::assistant("").with_tool_calls(
                self.calls
                    .iter()
                    .map(|(name, args)| lokai_inference::ToolCall {
...
async fn sub01_turn_uses_compiled_instructions() {
    ...
    let inv = lokai_domain::AgentInvocation {
...
fn sub01_orchestrator_passes_invocation() {
    let src = read(engine_root().join("mantle/lokai-orchestrator/src/turn.rs"));
...
fn sub01_prompt_task_law_gone_from_core() {
    ...
    assert!(
        !Path::new(&engine_root().join("core/lokai-core/src/task.rs")).exists(),
...
fn sub01_infer_never_constructs_agent() {
    let node = engine_root().join("mantle/lokai-node/src");
    fn mentions_agent(root: &Path) -> bool {
        ...
        for entry in fs::read_dir(root).unwrap() {
            ...
            if let Ok(src) = fs::read_to_string(&path) {
                if src.contains("lokai_core::Agent") || src.contains("Agent::new") {
                    return true;
                }
            }
        }
