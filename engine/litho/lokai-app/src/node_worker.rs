...
impl Application {
    pub async fn run_node_enroll() -> Result<()> {
        ...
        let subscriber = lokai_telemetry::init_subscriber(mode);
        ...
        let identity = load_or_create_tls_identity(
            &worker_store,
            &lokai_secrets::key_storage::PlatformKeyStorage,
        )?;
        ...
        lokai_secrets::private_file::write_new(&code_path, format!("{display}\n").as_bytes())
    ...
    pub async fn run_node_serve() -> Result<()> {
        ...
        let guard =
            std::sync::Arc::new(lokai_egress::EgressGuard::pinned_to_inference_url(&ollama));
        let client = Arc::new(lokai_capacity::OllamaInferenceClient::new(
            &ollama,
            Arc::new(lokai_inference::OllamaProvider::new(&ollama, guard)),
        ));
        let status =
            lokai_capacity::capacity_status_for_worker_path(worker_db.clone(), client, None).await;
        tracing::info!(
        ...
        if status.0.doctor == lokai_capacity::CapacityDoctorStatus::Degraded {
            tracing::warn!(
