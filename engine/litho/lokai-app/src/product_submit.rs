...
use lokai_core::{Conversation, ExactTokenizer, HeuristicTokenizer, Tokenizer};
use lokai_egress::EgressGuard;
use lokai_inference::InferenceProvider;
use lokai_memory::RecoverMutex;
use lokai_orchestrator::SpawnLimits;
...
pub struct TurnBind {
    ...
    pub runtime: Arc<lokai_runtime::EngineRuntime>,
    ...
    pub policy: Arc<lokai_policy::PolicyEngine>,
    guard: Mutex<Option<Arc<EgressGuard>>>,
    ollama_base: Mutex<String>,
    provider: Mutex<Option<Arc<dyn InferenceProvider>>>,
    compute_broker: Mutex<Option<Arc<lokai_broker::DefaultComputeBroker>>>,
    pooled: Mutex<Option<Arc<lokai_inference::PooledProvider>>>,
    compute_registry: Mutex<Option<Arc<lokai_inference::ComputeTargetRegistry>>>,
...
impl TurnBind {
    pub fn new(
        runtime: Arc<lokai_runtime::EngineRuntime>,
    ...
    pub fn set_fabric_plane(
        &self,
        pooled: Option<Arc<lokai_inference::PooledProvider>>,
        compute_registry: Option<Arc<lokai_inference::ComputeTargetRegistry>>,
    ) {
    ...
    pub fn compute_broker(&self) -> Option<Arc<lokai_broker::DefaultComputeBroker>> {
...
impl Application {
    pub fn bind_inference(
        ...
        compute_broker: Option<Arc<lokai_broker::DefaultComputeBroker>>,
    ) {
    ...
    pub fn install_compute_plane(
        ...
        provider: Arc<dyn InferenceProvider>,
        compute_broker: Option<Arc<lokai_broker::DefaultComputeBroker>>,
        fabric_remotes: &[Arc<lokai_fabric_client::RemoteNodeProvider>],
    ) {
    ...
    pub fn egress_allow_rules(&self) -> Vec<lokai_egress::AllowRule> {
    ...
    pub async fn fabric_status(&self) -> Result<serde_json::Value, AppError> {
        ...
        if let Some(ref store) = self.turn.store {
            let cap = store
                .read_sync(lokai_capacity::status_for_node)
                .unwrap_or_default();
            lokai_capacity::enrich_local_node_capacity(&mut snap, &cap);
        }
        let mut value =
    ...
    pub async fn set_fabric_worker_trust(
        ...
        let trust = lokai_domain::WorkerTrust::parse(trust_str).ok_or_else(|| {
            ...
            .or_else(|| {
                self.turn.store.as_ref().and_then(|store| {
                    store
                        ...
                        .and_then(|res| res.ok())
                        .flatten()
                        .and_then(|s| lokai_domain::WorkerTrust::parse(&s))
                })
            })
    ...
    pub fn reclassify_session_live(&self, session_id: &str, data_class: &str) {
        if let Some(class) = lokai_policy::parse_data_class(data_class) {
            if let Ok(live) = self.sessions.live(session_id) {
    ...
    pub fn reload_inference_services(&self) -> Option<lokai_capacity::InferenceDefaults> {
    ...
    pub async fn daemon_capacity_status(&self) -> Result<lokai_capacity::CapacityStatus, AppError> {
        let guard = self.turn.guard();
        let base = self.turn.ollama_base();
        let provider = Arc::new(lokai_inference::OllamaProvider::new(&base, guard));
        let client: Arc<dyn lokai_capacity::InferenceClient> =
            Arc::new(lokai_capacity::OllamaInferenceClient::new(&base, provider));
        let ver = client.version().await;
        let result = self
            .capacity
            .get_capacity_status(crate::commands::CapacityStatusCommand {
                client,
                node_id: lokai_capacity::LOCAL_NODE_ID.to_string(),
                ollama_version: ver,
            })
            .await?;
        Ok(result.status)
    ...
    pub async fn daemon_capacity_doctor(
        ...
        (
            lokai_capacity::CapacityStatus,
            lokai_capacity::CapacityDiagnosis,
        ),
        AppError,
    > {
        let guard = self.turn.guard();
    ...
    pub fn submit_chat_turn(&self, mut cmd: RunTurnCommand) -> Result<(), AppError> {
        ...
        let cap = self
            .capacity
            .snapshot_admission_status(lokai_capacity::LOCAL_NODE_ID);
