...
use lokai_orchestrator::ChildJob;
use lokai_run::{job_input_digest, RunSupervisor};
...
pub use lokai_run::{ExecutionPolicy, FinalizationEffectDriver, FinalizationPolicy};
...
pub(crate) type ActiveTurnRun = lokai_run::ManagedBinding;
...
pub struct DefaultRunService {
    ...
    artifacts: Arc<dyn lokai_domain::artifact::ArtifactStore>,
    identity_supplier: IdentitySupplier,
    session_dispatches: Arc<Mutex<HashMap<String, lokai_run::DispatchId>>>,
    session_runs: Arc<Mutex<HashMap<String, RunId>>>,
    pub(crate) managed: Arc<lokai_run::ManagedRunService>,
    approvals: Arc<Mutex<Option<Arc<dyn crate::approval::ApprovalService>>>>,
    runtime: Arc<Mutex<Option<Arc<lokai_runtime::EngineRuntime>>>>,
}
...
impl DefaultRunService {
    pub fn new(
        ...
        let identity_supplier: IdentitySupplier = Arc::new(|job_input: &str| {
            ...
            let id = AgentIdentity {
                id: lokai_domain::IdentityId::new("identity_default"),
                owning_application: "lokai-app".into(),
        ...
        let managed = Arc::new(
            lokai_run::ManagedRunService::new(
                supervisor.clone(),
                store.clone(),
                artifacts.clone(),
                policy.clone(),
        ...
        managed.attach_hooks(Arc::new(super::run_service_hooks::ProductRunHooks {
            ...
            scanner: lokai_secrets::ScannerEngine::default_engine(),
            events: events.clone(),
    ...
    pub fn managed(&self) -> &Arc<lokai_run::ManagedRunService> {
        &self.managed
    }
    ...
    pub fn attach_runtime(&self, runtime: Arc<lokai_runtime::EngineRuntime>) {
...
impl RunService for DefaultRunService {
    ...
    async fn plan_turn(&self, cmd: &RunTurnCommand) -> Result<RunTurnPlan, AppError> {
        ...
        lokai_telemetry::fault::inject_fault("after_turn_plan");
            ...
            if let Some(s) = &self.store {
                let persist = s
                    .write({
                        ...
                        move |db| {
                            let session_host =
                                lokai_orchestrator::SessionHost::new(workspace_root, policy);
