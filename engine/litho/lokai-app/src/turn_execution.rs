...
use lokai_domain::secrets::{OutboundRedaction, OutboundRedactionSink};
use lokai_domain::{
    AgentInvocation, AttemptId, CandidateOutcome, CodeIndexOpen, LspSessionOpen, TaskId,
};
use lokai_inference::FabricCallMeta;
...
use lokai_context::workspace::{build_production_context_compiler_injected, ContextFsHooks};
use lokai_runtime::{
...
struct AppRootExecute {
    ...
    approval_hook: lokai_core::ApprovalHook,
}
...
use lokai_domain::CompletionKind;
use lokai_secrets::ScannerEngine;
use lokai_tools::{EnforcementLevel, Tools, Workspace};
...
struct ToolsFinalizationDriver(std::sync::Arc<lokai_tools::Tools>);
...
impl FinalizationEffectDriver for ToolsFinalizationDriver {
    ...
    fn run_verify(
        ...
        if ok {
            ...
            let hint = lokai_tools::summarize_verify_failure(&output);
            Err((output, hint))
        }
    }
...
fn product_code_index(index_db: &Option<std::path::PathBuf>) -> Option<Arc<dyn CodeIndexOpen>> {
    index_db
        .as_ref()
        .map(|_| Arc::new(lokai_index::FilesystemCodeIndex) as Arc<dyn CodeIndexOpen>)
}
...
fn product_lsp_open(
    ...
    let consumer: Arc<dyn lokai_domain::CapabilityConsumer> = runtime.capability_store().clone();
...
pub(crate) struct TurnExecutionHost {
    ...
    pub store: Option<lokai_memory::SharedStore>,
    ...
    #[allow(dead_code)]
    pub llm_router: bool,
    pub plan: lokai_orchestrator::SessionStartPlan,
    ...
    pub compute_broker: Option<Arc<lokai_broker::DefaultComputeBroker>>,
...
pub trait AuditFactory: Send + Sync {
    fn session_audit(&self, session_id: &str, agent_id: &str) -> Box<dyn lokai_core::AuditSink>;
}
...
fn redact_step_text(
    ...
    let scanner = scanner?;
    match lokai_secrets::redact_text_sync(scanner, &text) {
        Ok((out, hit)) => {
            if !hit {
                return Some(text);
            }
        ...
        Err(_) => Some(lokai_secrets::SCAN_FAILED_PLACEHOLDER.to_string()),
    }
}
...
fn redact_step_json(
    ...
    let scanner = scanner?;
    match lokai_secrets::redact_json_value(scanner, &value) {
        Ok((out, _)) => Some(out),
        Err(_) => Some(serde_json::Value::String(
            lokai_secrets::SCAN_FAILED_PLACEHOLDER.to_string(),
        )),
    }
}
...
pub(crate) fn parse_spawn_role(role: &str) -> Result<lokai_orchestrator::RoleId, AppError> {
    CodingPack
        .parse(role)
...
fn composition_fs_hooks() -> ContextFsHooks {
    ContextFsHooks {
        ...
        run_git: Arc::new(|root, args| {
            let pe = lokai_tools::coding_executor(root, EnforcementLevel::Sandboxed);
...
struct ExecutionProjectionAudit {
    inner: Box<dyn lokai_core::AuditSink>,
    store: Option<lokai_memory::SharedStore>,
    session_id: String,
    agent_id: String,
    plan_user: String,
...
impl lokai_core::AuditSink for ExecutionProjectionAudit {
...
fn build_agent(
...
) -> Result<(Agent, lokai_domain::AgentInvocation), AppError> {
    ...
    let stable_catalog = if !build.spawned && build.dynamic_spec.is_none() {
        let catalog_tools =
            parts.turn_tools.as_ref().clone().with_orchestration(
                host.orchestration == lokai_orchestrator::OrchestrationMode::Auto,
            );
        let catalog = lokai_domain::ToolHost::advertisements(&catalog_tools)
            .into_iter()
            .map(|t| lokai_inference::ToolSchema::function(&t.name, &t.description, t.parameters))
            .collect::<Vec<_>>();
        let mut active = lokai_domain::ToolHost::advertisements(&tools)
            .into_iter()
            .map(|t| t.name)
            .collect::<Vec<_>>();
        active.sort();
    ...
    let inner = host
        ...
        .unwrap_or_else(|| Box::new(NullAudit) as Box<dyn lokai_core::AuditSink>);
    let audit = if turn.is_some() {
        ...
        }) as Box<dyn lokai_core::AuditSink>
    } else {
        inner
    };
    ...
    let process_broker = Some(match &host.compute_broker {
        Some(broker) => {
            let inner: Arc<dyn lokai_domain::sinks::ProcessBroker> =
                Arc::new(tools.executor().clone());
            Arc::new(lokai_broker::BrokerGatedProcessBroker::new(
                broker.clone(),
                inner,
            )) as Arc<dyn lokai_domain::sinks::ProcessBroker>
        }
        None => Arc::new(tools.executor().clone()) as Arc<dyn lokai_domain::sinks::ProcessBroker>,
    });
    ...
    let context_compiler: Option<Arc<dyn lokai_domain::ContextCompiler>> =
        workspace_root.as_ref().map(|root| {
            let compiler: Arc<dyn lokai_domain::ContextCompiler> =
                build_production_context_compiler_injected(
                    ...
                    host.index_db.as_ref().map(|_| {
                        Arc::new(lokai_index::IndexTextSkeleton)
                            as Arc<dyn lokai_domain::TextSkeleton>
                    }),
    ...
    if let (Some(owner), Some(compiler)) = (&host.live_session, &context_compiler) {
        ...
        owner
            .register_context_compiler(lokai_domain::SessionId::new(&host.session_id), compiler)?;
    }
...
pub(crate) async fn execute_turn(
    ...
    lokai_telemetry::propagation::scope_context(
        lokai_telemetry::TraceContext::default(),
...
async fn execute_turn_scoped(
    ...
    let task_timer = lokai_telemetry::StageTimer::start(lokai_telemetry::PerfStage::TaskTurn);
    ...
    lokai_telemetry::inject_turn_context(
        &cmd.session_id,
        &turn_plan.run_id.0,
...
async fn run_turn_body(
    ...
    let fs_index = lokai_index::FilesystemCodeIndex;
    ...
    let llm_route = if llm_router && orchestration == OrchestrationMode::Auto {
        emit(
            ...
            ApplicationEvent::StageTransition {
                ...
                agent_id: lokai_orchestrator::ROOT_AGENT.to_string(),
            ...
            Some(|route, tier| {
                emit(
                    ...
                    ApplicationEvent::LogDiagnostic {
                        ...
                        agent_id: Some(lokai_orchestrator::ROOT_AGENT.to_string()),
    ...
    *spawn_serial = spawn_serial_cell.load(Ordering::SeqCst);
    ...
    let finalization_timer =
        lokai_telemetry::StageTimer::start(lokai_telemetry::PerfStage::Finalization);
...
pub(crate) async fn execute_spawn(
    ...
    let child_task_id = runs
        ...
        .unwrap_or_else(|| lokai_domain::TaskId::new(format!("task_{}", spawn_attempt_id.0)));
