...
use lokai_core::Conversation;
use lokai_domain::{RunId, TaskId, WorkspaceBinding};
use lokai_memory::RecoverMutex;
use lokai_orchestrator::{OrchestrationMode, SessionStartPlan, SpawnSessionTrack};

use crate::errors::AppError;

struct TurnState {
    context_compilers: Vec<(
        lokai_domain::SessionId,
        std::sync::Weak<dyn lokai_domain::ContextCompiler>,
    )>,
    in_flight: bool,
    closing: bool,
...
impl LiveSession {
    ...
    pub fn set_plan_data_class(&self, class: lokai_domain::DataClass) {
        self.plan.lock_recover().data_class = class;
    }

    pub fn take_conversation(&self) -> Result<Conversation, AppError> {
        lokai_memory::mutex_lock(&self.conversation)
            .take()
    ...
    pub(crate) fn register_context_compiler(
        &self,
        session: lokai_domain::SessionId,
        compiler: &Arc<dyn lokai_domain::ContextCompiler>,
    ) -> Result<(), AppError> {
        let mut state = self.turn_state.lock_recover();
        if state.closing {
            compiler
...
pub fn capacity_gate_warning(
    session_model: &str,
    status: &lokai_capacity::CapacityStatus,
) -> Option<CapacityGateWarning> {
    if status.profile_model.is_none() && status.profile_base_model.is_none() {
        return None;
    }
    let doctor = lokai_capacity::doctor_status_str(status.doctor);
    let same = lokai_capacity::session_matches_profile(
        session_model,
...
pub fn admit_chat_turn(
    ...
    capacity_busy: bool,
    live: &LiveSession,
    capacity: Option<&lokai_capacity::CapacityStatus>,
) -> TurnAdmitDecision {
    if draining {
...
mod tests {
    ...
    use lokai_orchestrator::SessionStartPlan;
    ...
    fn closing_revokes_registered_compilers_and_rejects_late_registration() {
        ...
        impl lokai_domain::ContextCompiler for Compiler {
            fn invalidate_session(&self, session: &lokai_domain::SessionId) -> Result<(), String> {
            ...
            async fn compile(
                &self,
                _: lokai_domain::ContextCompileRequest,
        ...
        let compiler: Arc<dyn lokai_domain::ContextCompiler> = Arc::new(Compiler(calls.clone()));
        session
            .register_context_compiler(lokai_domain::SessionId::new("session-owner"), &compiler)
            .unwrap();
        session.begin_close().unwrap();
    ...
    fn degraded_status(profile: &str, base: &str) -> lokai_capacity::CapacityStatus {
        lokai_capacity::CapacityStatus {
            completed: true,
            stale: false,
            doctor: lokai_capacity::CapacityDoctorStatus::Degraded,
    ...
    fn healthy_profile_admits_without_warning() {
        let s = live();
        let status = lokai_capacity::CapacityStatus {
            completed: true,
            stale: false,
            doctor: lokai_capacity::CapacityDoctorStatus::Healthy,
            active_profile_id: Some("p1".into()),
            active_profile_label: Some("lab".into()),
            hardware_summary: None,
            gates_ok: true,
    ...
    fn no_profile_does_not_warn() {
        let s = live();
        let status = lokai_capacity::CapacityStatus::default();
        assert_eq!(
            admit_chat_turn(false, false, &s, Some(&status)),
            TurnAdmitDecision::Admit {
                capacity_warning: None
