...
use lokai_egress::{Action, EgressEvent, EgressGuard};
use lokai_fabric_client::fabric_request;
...
impl Application {
    ...
    pub async fn ollama_evict(&self) -> Result<String, AppError> {
        ...
        let provider = lokai_inference::OllamaProvider::new(base.trim_end_matches('/'), guard);
        let mut failed = 0;
        for m in models {
...
fn format_ps(v: &serde_json::Value) -> String {
    ...
    for m in models {
        ...
        let expires = m
            ...
            .and_then(|v| v.as_str())
            .unwrap_or("unknown");
        let gpu_pct = lokai_capacity::gpu_residency_pct_from_ps(size, vram)
