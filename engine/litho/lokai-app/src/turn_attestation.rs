...
use lokai_domain::artifact::{ArtifactDeclaration, ArtifactKind, ArtifactStore, RetentionPolicy};
use lokai_domain::classify::DataClass;
use lokai_domain::ids::{ArtifactId, AttemptId, RunId, TaskId};
use lokai_domain::CandidateOutcome;
use std::sync::Arc;
...
pub fn encode_candidate_bytes(outcome: &CandidateOutcome) -> Result<Vec<u8>, AppError> {
    ...
    .map_err(|e| AppError::InternalViolation(e.to_string()))
}
...
pub async fn seal_output_set(
    ...
    let meta = writer
        ...
        .await
        .map_err(|e| AppError::PersistenceFailed(e.to_string()))?;
    lokai_artifact::verify_stored_content(artifacts.as_ref(), &meta.artifact_id)
        .await
        .map_err(|e| AppError::PersistenceFailed(e.to_string()))?;
    Ok(SealedTurn {
...
mod tests {
    ...
    fn identical_output_encodes_identically() {
        let outcome = CandidateOutcome::Completed {
            summary: "done".into(),
            kind: lokai_domain::CompletionKind::Finish,
        };
    ...
    #[test]
    fn placeholder_digest_absent_from_production() {
        let needle = ["sha256:", "turn_ok"].concat();
        let run_src = include_str!("run_service.rs");
        assert!(
            !run_src.contains(&needle),
            "production complete_turn must not use the placeholder digest"
        );
        let att_src = include_str!("turn_attestation.rs");
        let production = att_src.split("#[cfg(test)]").next().unwrap();
        assert!(
            !production.contains(&needle),
            "turn attestation production path must not use the placeholder digest"
        );
    }
}
