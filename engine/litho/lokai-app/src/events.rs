...
pub enum ApplicationEvent {
    ...
    ApprovalRequest {
        ...
        tool: String,
        args: serde_json::Value,
        missing_controls: Vec<lokai_core::ConfinementWarning>,
    ...
    ContextSnapshot {
        ...
        estimated: bool,
        data_class: Option<lokai_domain::DataClass>,
    },
    DispatchPlacement {
        session_id: String,
        agent_id: String,
...
pub fn terminal_status(outcome: &lokai_domain::CandidateOutcome) -> String {
    match outcome {
        lokai_domain::CandidateOutcome::Completed { .. } => "ok".into(),
        lokai_domain::CandidateOutcome::Canceled { .. } => "canceled".into(),
        lokai_domain::CandidateOutcome::Failed { .. } => "error".into(),
        _ => "ok".into(),
    }
}

pub fn outcome_error(outcome: &lokai_domain::CandidateOutcome) -> Option<String> {
    match outcome {
        lokai_domain::CandidateOutcome::Failed { message } => Some(message.clone()),
        lokai_domain::CandidateOutcome::Canceled { reason } => Some(reason.clone()),
        _ => None,
    }
}
