...
use lokai_enroll::{allow_fabric_workers, complete_enrollment, KeyPair};
use lokai_memory::{Store, WorkerEnrollmentRow};
use lokai_node::push_revoke;
...
pub fn load_or_create_coordinator(store: &Store, data_dir: &Path) -> Result<KeyPair, AppError> {
    ...
    lokai_secrets::private_file::write_new(&path, kp.signing_bytes().as_ref())
