...
use lokai_inference::hosted::registry::{
...
impl Application {
    pub async fn bootstrap_cli(params: CliBootstrapParams) -> Result<CliBootstrapOutput, AppError> {
        ...
        let defaults = store
            ...
            .unwrap_or_else(lokai_capacity::InferenceDefaults::fallback);
        ...
        let (installed, active_endpoint) = if let Some(ref hosted) = hosted_setup {
            ...
            let provider = Arc::new(lokai_inference::OllamaProvider::new(
                &params.ollama,
                guard.clone(),
            ));
        ...
        let ws = lokai_tools::Workspace::new(Path::new(&params.workspace)).map_err(|e| {
        ...
        let verify_resolved =
            lokai_tools::resolve_verify_cmd(verify_explicit, Path::new(&workspace_root));
        ...
        let orchestration = lokai_orchestrator::OrchestrationMode::parse(&params.orchestrate);
        let critic_enabled = orchestration == lokai_orchestrator::OrchestrationMode::Auto
            && !params.no_critic
            && !params.explain;
        ...
        let _ = lokai_tools::ensure_session_worktree(
    ...
    pub async fn bootstrap_offline(workspace: Option<&str>) -> Result<Arc<Application>, AppError> {
        ...
        let ws_root = match workspace {
            Some(w) => lokai_tools::Workspace::new(Path::new(w))
...
struct HostedSetup {
    model: String,
    endpoint: String,
    provider: Arc<dyn lokai_inference::InferenceProvider>,
}

fn resolve_hosted_setup(
    params: &CliBootstrapParams,
    selected_model: &str,
    guard: Arc<lokai_egress::EgressGuard>,
...
pub fn open_default_audit_store() -> Option<lokai_memory::SharedStore> {
    let dirs = directories::ProjectDirs::from("", "", "lokai")?;
    let path = dirs.data_dir().join("lokai.db");
    lokai_memory::SharedStore::open(&path, 5).ok()
}

fn resolve_models(
    explicit: Option<&str>,
    hard: Option<&str>,
    defaults: &lokai_capacity::InferenceDefaults,
) -> (String, String) {
...
mod model_tests {
    ...
    fn explicit_model_including_default_name_beats_saved_profile() {
        let mut defaults = lokai_capacity::InferenceDefaults::fallback();
        defaults.model_fast = "saved-fast".into();
        defaults.model_hard = "saved-hard".into();
        assert_eq!(
            resolve_models(Some(lokai_inference::DEFAULT_MODEL), None, &defaults),
            (
                lokai_inference::DEFAULT_MODEL.into(),
                lokai_inference::DEFAULT_MODEL.into()
            )
        );
        assert_eq!(
