...
use lokai_core::{Conversation, Step};
use lokai_domain::{DataClass, DisclosureTier};
use lokai_memory::RecoverMutex;
use lokai_orchestrator::{RoleId, SessionStartPlan, SpecialistPack};
...
fn aud01_defect_bh_appr_loss_waiter_is_process_local() {
    ...
    let live = LiveSession::new(
        Conversation::new(),
        lokai_orchestrator::OrchestrationMode::Single,
        false,
        false,
        32,
        "fast".into(),
...
async fn aud01_bh_multi_chat_conversation_survives_turns() {
    ...
    live.restore_conversation(Conversation::from_audit_messages(vec![
        lokai_inference::Message::user("hello history"),
    ]));
...
fn aud01_bh_cmp_infer_never_constructs_agent_inventory() {
    ...
    assert!(!src.contains("lokai_core::Agent"));
