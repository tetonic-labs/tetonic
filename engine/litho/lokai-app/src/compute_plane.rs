...
use lokai_fabric_client::{CapabilityRegistry, RemoteNodeProvider};
use lokai_inference::{
    AuthorizedComputeTarget, ComputeTargetRegistry, OllamaProvider, PooledProvider,
};
use lokai_memory::{SharedStore, Store};
use lokai_policy::{PolicyDispatchGuard, PolicyEngine};
...
pub struct ComputePlaneRequest {
    ...
    pub artifact_store: Arc<dyn lokai_domain::artifact::ArtifactStore>,
    pub store: Option<SharedStore>,
    pub coordinator: Option<Arc<KeyPair>>,
    pub placement_sink: Option<Arc<dyn lokai_inference::DispatchPlacementSink>>,
...
fn build_pooled_provider(
    ...
    artifact_store: Arc<dyn lokai_domain::artifact::ArtifactStore>,
    placement_sink: Option<Arc<dyn lokai_inference::DispatchPlacementSink>>,
    previous_pooled: Option<Arc<PooledProvider>>,
    coordinator: Arc<KeyPair>,
    owner: lokai_memory::OwnerIdentityRow,
    ...
    let dispatch_guard: Arc<dyn lokai_inference::DispatchGuard> =
        ...
        .map(|t| {
            Arc::new(
                ...
                .with_disposition_persistence(std::sync::Arc::new(
                    lokai_fabric_client::StoreDispositionPersist::new(trust_store.clone()),
                )),
            )
        })
        .collect();
    let remotes: Vec<Arc<dyn lokai_inference::FabricNodeProvider>> = concrete_remotes
        .iter()
        .map(|r| r.clone() as Arc<dyn lokai_inference::FabricNodeProvider>)
        .collect();
    ...
    let mut pooled = PooledProvider::new_with_registry(
        ...
        remotes,
        policy_epoch,
        Arc::new(lokai_inference::ActiveJobRegistry::new()),
    )
