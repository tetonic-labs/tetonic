...
use lokai_sandbox::{apply_executable, profile_for_class, ProcessMode, SyncLongLivedService};
use lokai_tools::process_executor::ProcessExecutor;
use lokai_tools::sandbox_bridge::{format_sandbox_audit, process_class_for_lsp};
...
impl LspProcessLauncher for SandboxLspLauncher {
    fn spawn(&self, spec: &ServerSpec, root: &Path) -> Result<SpawnedLspProcess, LspError> {
        ...
        tracing::info!(
            "lokai_sandbox: lsp_spawn execution_id={} program={} label={} audit={}",
            execution_id,
