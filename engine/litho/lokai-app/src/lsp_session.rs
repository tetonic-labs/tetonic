...
use lokai_domain::{LspSession, LspSessionOpen, ToolOutcome};
use lokai_lsp::{available_languages, lsp_enabled_by_env, LspPool};
use lokai_tools::process_executor::{coding_executor, EnforcementLevel};
...
impl SandboxLspSessionOpen {
    pub fn new(consumer: Option<Arc<dyn lokai_domain::CapabilityConsumer>>) -> Self {
        Self { consumer }
    }
}
...
impl LspSessionOpen for SandboxLspSessionOpen {
    fn open(&self, root: &Path) -> Result<Box<dyn LspSession>, String> {
        ...
        let launcher: Option<Arc<dyn lokai_lsp::LspProcessLauncher>> = if executor.uses_os_sandbox()
        {
