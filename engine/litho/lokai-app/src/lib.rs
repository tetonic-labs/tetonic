...
pub use lokai_core::ConfinementWarning;
pub use lokai_inference::{
...
pub use lokai_memory::SharedStore;
pub use lokai_orchestrator::{next_child_agent_id, ROOT_AGENT};
pub use lokai_policy::data_class_name;
pub use lokai_secrets::ScannerEngine;
pub use lokai_telemetry;
...
impl lokai_run::RunEventHook for RunCommitEventHook {
    fn on_run_committed(&self, result: &lokai_domain::RunCommandResult) {
        tracing::debug!(
...
pub(crate) fn build_supervisor(
...
) -> Arc<dyn lokai_run::RunSupervisor> {
    ...
    let sup = lokai_run::DurableRunSupervisor::new(store).with_hook(Arc::new(RunCommitEventHook));
    Arc::new(sup)
}
...
pub struct Application {
    ...
    pub(crate) supervisor: Arc<dyn lokai_run::RunSupervisor>,
...
impl Application {
    ...
    pub fn new(deps: ApplicationDependencies) -> Self {
        ...
        Self {
            ...
            turn: product_submit::TurnBind::new(
                ...
                deps.index_db,
                String::new(),
                lokai_capacity::InferenceDefaults::fallback().num_ctx,
                deps.event_sink,
                deps.policy.clone(),
            ),
        }
    ...
    pub fn add_secret_rule(
        &self,
        scanner: &lokai_secrets::ScannerEngine,
        pattern: &str,
    ) -> Result<(), errors::AppError> {
        let regex = regex::Regex::new(pattern)
            .map_err(|_| errors::AppError::InvalidRequest("Invalid regex pattern".into()))?;
        let detector = lokai_secrets::detectors::RegexDetector {
            rule_id: format!("user_rule_{}", lokai_memory::new_id("rule")),
            version: 1,
            regex,
            kind: lokai_secrets::types::SecretKind::ProviderToken,
            confidence: lokai_secrets::types::FindingConfidence::Confirmed,
        };
...
pub fn generate_rpc_token() -> String {
    let kp = lokai_enroll::KeyPair::generate();
...
fn parse_secret_override_scope(
    kind: &str,
    id: Option<&str>,
) -> Result<lokai_secrets::OverrideScope, errors::AppError> {
    match kind {
        "global" | "" => Ok(lokai_secrets::OverrideScope::Global),
        "session" => {
            let id = id.filter(|s| !s.is_empty()).ok_or_else(|| {
                errors::AppError::InvalidRequest("session scope requires scope_id".into())
            })?;
            Ok(lokai_secrets::OverrideScope::Session(id.to_string()))
        }
        "project" => {
            let id = id.filter(|s| !s.is_empty()).ok_or_else(|| {
                errors::AppError::InvalidRequest("project scope requires scope_id".into())
            })?;
            Ok(lokai_secrets::OverrideScope::Project(id.to_string()))
        }
