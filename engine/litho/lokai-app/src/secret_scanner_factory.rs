...
use lokai_memory::{SecretOverrideScope, SharedStore, Store};
use lokai_secrets::detectors::default_detectors;
use lokai_secrets::override_scope::OverrideScope;
...
pub fn install_shared_scanner_from_store(store: &Option<SharedStore>) {
    ...
    if let Ok(engine) = s.read_sync(scanner_engine_from_store) {
        lokai_secrets::install_shared_scanner(engine);
    }
}
...
pub fn artifact_scan_policy(store: &Option<SharedStore>) -> lokai_artifact::ScanPolicy {
    let scanner = scanner_from_shared_store(store);
    lokai_artifact::ScanPolicy::Scan(Arc::new(move |text: &str| -> bool {
