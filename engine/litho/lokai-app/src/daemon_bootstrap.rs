...
use anyhow::Result;
use lokai_capacity::{CapacityStatus, LOCAL_NODE_ID};
use lokai_egress::EgressGuard;
use lokai_enroll::KeyPair;
use lokai_inference::{DispatchPlacementSink, InferenceProvider, OllamaProvider};
...
impl Application {
    pub async fn bootstrap_daemon(
        ...
        let startup_timer = lokai_telemetry::StageTimer::start(lokai_telemetry::PerfStage::Startup);
        ...
        let mut open_index: Option<lokai_index::Index> = None;
        if let Some(dirs) = directories::ProjectDirs::from("", "", "lokai") {
            let db_path = dirs.data_dir().join("index.db");
            if let Ok(idx) = lokai_index::Index::open(&db_path) {
        ...
        if let Some(idx) = open_index {
            ...
            let attempt_id =
                lokai_domain::ids::AttemptId::new(format!("att_index_{}", uuid::Uuid::new_v4()));
            let index_req = lokai_broker::ComputeRequest {
                run_id: lokai_domain::RunId::new(format!("run_index_{}", uuid::Uuid::new_v4())),
                task_id: lokai_domain::TaskId::new("task_index_workspace"),
                task_version: 1,
                attempt_id,
                job_kind: lokai_fabric_protocol::JobKind::IndexShard,
                input_artifacts: vec![],
                input_digest: lokai_domain::ContentDigest::new(format!("index:{}", workspace_root)),
                workspace_version: None,
                data_class: lokai_domain::DataClass::RepositorySource,
                placement_decision: lokai_broker::PlacementDecisionReference {
                ...
                resource_request: lokai_broker::profile_for(
                    &lokai_fabric_protocol::JobKind::IndexShard,
                )
                .default_resources,
                deadline: lokai_broker::DeadlinePolicy::default(),
                retry_policy: lokai_broker::RetryPolicyReference::default(),
                verification_policy: lokai_domain::VerificationPolicyReference {
                    policy_id: "structural".into(),
                },
                priority: lokai_broker::ComputePriority::Background,
                trace_context: lokai_domain::TraceContext::default(),
                speculative: false,
                project_id: None,
        ...
        let tools: Vec<String> = {
            ...
            if let Some(db) = &index_db {
                t = t
                    .with_index(db.clone())
                    .with_code_index_open(Arc::new(lokai_index::FilesystemCodeIndex));
            }
            if let Some(ref s) = store {
                let mem_path = s
        ...
        let capacity_status = if let Some(ref s) = store {
            let client: Arc<dyn lokai_capacity::InferenceClient> =
                Arc::new(lokai_capacity::OllamaInferenceClient::new(
                    &ollama_base,
                    Arc::new(OllamaProvider::new(&ollama_base, guard.clone())),
                ));
            let ver = client.version().await;
            let (status, _) =
                lokai_capacity::capacity_status_for_store(s, LOCAL_NODE_ID, &client, ver).await;
            Some(status)
        } else {
            None
        };
