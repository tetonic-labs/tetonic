...
use tetonic_lsp::{Lang, LspPool, LspSession, ServerSpec};

struct TrackedLauncher(std::sync::Arc<std::sync::atomic::AtomicBool>);
struct TrackedProcess {
    inner: tetonic_lsp::SpawnedLspProcess,
    stopped: std::sync::Arc<std::sync::atomic::AtomicBool>,
}
impl tetonic_lsp::LspProcessHandle for TrackedProcess {
...
impl tetonic_lsp::LspProcessLauncher for TrackedLauncher {
    fn spawn(
        &self,
        spec: &ServerSpec,
        root: &std::path::Path,
    ) -> Result<tetonic_lsp::SpawnedLspProcess, tetonic_lsp::LspError> {
        ...
        Ok(tetonic_lsp::spawned_from_io(
            stdin,
            stdout,
...
fn stalled_native_stdin_times_out_and_session_is_not_reused() {
    ...
    assert!(matches!(
        ...
        Err(tetonic_lsp::LspError::Timeout)
    ));
    assert!(started.elapsed() < std::time::Duration::from_secs(30));
    assert_eq!(session.lifecycle(), tetonic_lsp::LspLifecycle::Unhealthy);
    assert!(
    ...
    assert_eq!(session.lifecycle(), tetonic_lsp::LspLifecycle::Stopped);
...
fn mock_goto_definition_and_diagnostics() {
    ...
    assert!(session.is_healthy());
    assert_eq!(session.lifecycle(), tetonic_lsp::LspLifecycle::Ready);
