...
use tetonic_transaction::WorkspaceTransactionService;
...
pub(crate) fn enforce_live_workspace_version(
    ...
    let relevant: Vec<WorkspacePath> = authorized
        ...
        .into_iter()
        .collect();
    let live = tetonic_transaction::version::capture_workspace_version(root, &relevant)
...
impl RepositoryMutationService {
    ...
    fn apply_mutation_sync(&self, authorized: &AuthorizedAction) -> ExecutionOutcome {
        ...
        tetonic_telemetry::fault::inject_fault("during_workspace_mutation_staging");
        let outcome = self.run_authorized(&ws, authorized);
        tetonic_telemetry::fault::inject_fault("after_mutation");
        if outcome.ok {
    ...
    pub fn write_file(
        ...
        let before = if path.exists() {
            if tetonic_transaction::fs_ops::is_symlink_or_reparse(&path) {
    ...
    pub fn finish_verification_run(
        ...
        let record = tetonic_domain::VerificationRecord {
            command: command.to_string(),
            sandbox_policy: "constrained".into(),
            workspace_version: base,
            exit_status,
            output_digest: tetonic_transaction::digest_string(output),
            success,
    ...
    ) -> Result<tetonic_domain::CommitResult, ToolError> {
        let result = self
            .transactions
    ...
    pub fn preview(&self) -> Result<tetonic_domain::TransactionPreview, ToolError> {
        self.transactions
...
fn map_txn_err(e: tetonic_transaction::TransactionError) -> ToolError {
    let msg = e.to_string();
...
mod tests {
    ...
    impl CapabilityConsumer for DenyConsumer {
        fn authorize(&self, _: &AuthorizedAction) -> Result<(), tetonic_domain::CapabilityError> {
    ...
    fn denied_capability_blocks_write() {
        ...
        let authorized = AuthorizedAction {
            capability: IssuedCapability {
                capability_id: tetonic_domain::CapabilityId::new("cap_bad"),
    ...
    fn workspace_change_after_approval_rejects() {
        ...
        let wv = tetonic_transaction::version::capture_workspace_version(
            &dir,
        ...
        let authorized = AuthorizedAction {
            capability: IssuedCapability {
                capability_id: tetonic_domain::CapabilityId::new("cap_wv"),
                session_id: action.session_id.clone(),
                run_id: None,
                task_id: None,
                attempt_id: None,
    ...
    async fn mutation_sink_none_version_does_not_commit() {
        ...
        let authorized = AuthorizedAction {
            capability: IssuedCapability {
                capability_id: tetonic_domain::CapabilityId::new("cap_sink"),
                session_id: action.session_id.clone(),
                run_id: None,
                task_id: None,
                attempt_id: None,
