...
use lokai_domain::ids::{CommandId, EventId, RunId, TraceId};
use lokai_domain::{ContentDigest, RunCommandResult, RunEventEnvelope, RunSnapshot};
...
impl Store {
    ...
    pub fn commit_run_command(
        ...
        if let Some(command_id) = &event.command_id {
            let result = lokai_domain::RunCommandResult {
    ...
    pub fn compact_run_events_at_floor(
        ...
        let secret = serde_json::to_string(&lokai_domain::DataClass::Secret).unwrap();
        let sensitive = serde_json::to_string(&lokai_domain::DataClass::SensitiveSource).unwrap();
...
mod tests {
    ...
    use lokai_domain::ids::{EventId, TraceId};
    use lokai_domain::{ContentDigest, EventActor, EventType, RunId, RunState, SessionId};
