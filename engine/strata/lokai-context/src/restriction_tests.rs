...
async fn cache_separates_scope_owner_retrieval_and_artifacts() {
    ...
    changes.push(req);
    let mut req = request.clone();
    req.session_id = lokai_domain::ids::SessionId::new("other-session");
    changes.push(req);
    let mut req = request.clone();
    req.run_id = lokai_domain::ids::RunId::new("other-run");
    changes.push(req);
    let mut req = request.clone();
    req.task_id = lokai_domain::ids::TaskId::new("other-task");
    changes.push(req);
    let mut req = request.clone();
    req.prior_artifacts
        .push(lokai_domain::ids::ArtifactId::new("other-artifact"));
    changes.push(req);
    for req in changes {
...
async fn expansion_preserves_exclusions_and_redacts_before_returning() {
    ...
    let compiler = ContextCompiler::new(Arc::new(provider)).with_secret_scanner(Arc::new(
        lokai_secrets::scanner::ScannerEngine::default_engine(),
    ));
