...
use lokai_domain::{classify::DataClass, ContextExpansionRequest};
use std::sync::{
...
impl crate::interfaces::SecretScanner for PausedScanner {
    async fn scan_and_redact(
        ...
        _: &str,
        _: Option<&str>,
    ) -> Result<lokai_domain::secrets::ScanOutcome, String> {
...
async fn session_revocation_rejects_inflight_and_future_expansion() {
    use lokai_domain::ContextCompiler as _;
    let (compiler, request, _) = fixture().await;
...
async fn each_owner_dimension_is_required_before_reading_or_consuming_use() {
    ...
    let port: &dyn lokai_domain::ContextCompiler = compiler.as_ref();
    for dimension in 0..3 {
        let mut foreign = request.clone();
        match dimension {
            0 => foreign.session_id = lokai_domain::SessionId::new("other"),
            1 => foreign.run_id = lokai_domain::RunId::new("other"),
            _ => foreign.task_id = lokai_domain::TaskId::new("other"),
        }
...
async fn simultaneous_valid_and_foreign_callers_cannot_steal_the_last_use() {
    ...
    compiler
        ...
        .max_uses = 1;
    let mut foreign = request.clone();
    foreign.session_id = lokai_domain::SessionId::new("other");
