...
pub mod tests {
    ...
    use lokai_domain::ids::{EvidenceId, ExpansionHandleId, RunId, SessionId, TaskId};
    ...
    pub(super) fn expansion_request(
        handle: &str,
        fingerprint: &str,
    ) -> lokai_domain::ContextExpansionRequest {
        lokai_domain::ContextExpansionRequest {
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // 19. Cache: same request → cache hit; changed workspace → cache miss
    // ─────────────────────────────────────────────────────────────────────────
    #[tokio::test]
    async fn cache_is_bounded_and_retains_recently_used_entries() {
        let cache = crate::cache::ContextCache::with_capacity(2);
        ...
        third.objective = "third".into();
        cache.insert(&first, &pack);
        cache.insert(&second, &pack);
        assert!(cache.get(&first).is_some());
        cache.insert(&third, &pack);
        assert_eq!(cache.len(), 2);
        assert!(cache.get(&second).is_none());
        assert!(cache.get(&first).is_some());
        assert!(cache.get(&third).is_some());
        let disabled = crate::cache::ContextCache::with_capacity(0);
        disabled.insert(&first, &pack);
        assert!(disabled.is_empty());
    }

    #[tokio::test]
    async fn test_cache_key_workspace_incompatibility() {
        ...
        let dummy_pack = {
            let compiler = ContextCompiler::new(Arc::new(MockProvider::default()));
            compiler.compile(req.clone()).await.unwrap()
        };
        cache.insert(&req, &dummy_pack);

        // Same request → hit
        assert!(cache.get(&req).is_some());

        // Different workspace fingerprint → miss
        let mut req2 = req.clone();
        req2.workspace_version = ws_version("changed_workspace");
        assert!(
            cache.get(&req2).is_none(),
            "different workspace must be a cache miss"
        );
    }

    // ─────────────────────────────────────────────────────────────────────────
    // 20. Cache invalidation on workspace change
    // ─────────────────────────────────────────────────────────────────────────
    #[tokio::test]
    async fn test_cache_invalidation_on_workspace_change() {
        use crate::cache::ContextCache;
        let cache = ContextCache::new();
        let req = base_request();
        ...
        cache.invalidate_workspace(&fp);

        assert_eq!(
            cache.len(),
            0,
            "workspace invalidation must flush all entries"
        );
        assert!(cache.get(&req).is_none());
    }
    ...
    async fn shadow_off_returns_no_error_on_success() {
        ...
        provider.text_results = vec![make_evidence(
            "fn shadow_target() {}",
            Some("src/shadow.rs"),
            lokai_domain::classify::DataClass::RepositorySource,
        )];
        let compiler = ContextCompiler::new(Arc::new(provider));
