...
Production configuration acceptance: engineer-configured telemetry destination, logging level/output and supported storage connection/path work without recompilation. Exercise invalid-config rejection, redacted effective configuration, exporter outage, storage outage and distributed reconnect/fencing. Verify [operator requirements](../../operator-configuration.md), already implemented with earlier subsystem tickets; do not introduce a second late-stage configuration system.
