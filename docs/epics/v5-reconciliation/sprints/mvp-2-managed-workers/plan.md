...
Status: in progress. Registered general jobs now submit through the application run service and existing managed executor, with stored-grant authorization, scoped output/replay and cancellation covered by integration tests. Configured runtime assembly exercises real workspace reads/writes through existing controls. Durable retries, host deadlines and one admitted activation per registered identity now share the existing run journal. Employee-facing launch transport, organization/team budgets and concurrency, bounded ingress/queues and the supported isolation matrix remain unfinished. Depends on the preceding MVP sprint; security and bounded admission are enforced incrementally, never deferred until final hardening. Tickets may be split into smaller implementation commits without weakening exit criteria.
...
## Registered identity admission and physical completion (2026-09-25)

A registered identity admits one activation at a time across managers sharing the same local SQLite database. The gate uses the stable identity, so another principal, context, request key or definition revision does not create another slot. Different identities may work concurrently. A matching authorized retry still returns its original receipt. A distinct request while occupied returns ExecutionCapacityExceeded, creates no run/attempt and starts no inference; it is not silently queued. Runtime preparation can still leave an unused audit history before admission fails.

The existing CreateRun transaction checks an indexed projection of registered identities under its Immediate write lock. Schema 41 adds only derived columns/index to run_projections; it creates no independent reservation store. The existing attempt record gains execution_quiesced, defaulting to false for historical records. A trusted manager records RecordAttemptQuiescence with the exact attempt lease identity after closing WorkScope and waiting for all its effect workers. The supervisor requires a terminal attempt and validates holder, lease ID and epoch. Terminal run state plus acknowledgements for every attempt releases capacity. CancelRun, a timeout, lease expiry or a missing local registry entry alone cannot release it.

Managed finalization now delivers terminal completion from its outer owner after the finalization lease has dropped and quiescence is recorded. Cancellation uses the same acknowledgement before removing ownership. Failed writes retain the hold; the live cancellation owner can retry. Scoped inspection/replay includes the acknowledgement. This is a trusted worker report about the existing WorkScope contract, not proof of sandbox containment or a new remote authentication protocol.

Upgrade/recovery: stop all writers before upgrading; mixed binary versions sharing a database are unsupported. The migration conservatively preserves existing registered runs without quiescence evidence, including terminal runs and multiple historical runs for one identity. Partial admission, crashed owners and historical ambiguous holds require explicit recovery work that is not yet implemented as a supported operator command. No TTL or canceled status guesses that it is safe to run another activation, and deleting run truth is not a supported recovery method. Migration 40 now checks its version marker so a real v40 database can upgrade without recreating execution-grant tables.

Remaining: durable org/team/principal ceilings and cumulative usage; bounded preparation/queues and fair scheduling; authenticated remote fencing/ownership and recovery; product launch transport and the other exit criteria above. The one-identity gate applies to registered activation admission, not legacy session runs or inference-hop accounting. It does not reserve an entire workspace across different identities or provide distributed coordination across separate databases.
