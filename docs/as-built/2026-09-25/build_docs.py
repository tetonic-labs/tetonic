...
Arrows are local calls/signals, not network delivery guarantees; the E-Stop packet itself arrives over WebSocket as described in execution. Canceled durable state, requested cancellation and actual process quiescence are separate milestones. World stop and application cancellation are different paths. Evidence: [[engine/core/tetonic-domain/src/work_scope.rs::pub struct WorkScope]], [[engine/litho/tetonicd/src/daemon.rs::pub async fn shutdown]], [[engine/mantle/tetonic-run/src/transition.rs::fn apply_cancel_run]], [[engine/core/tetonic-runtime/src/websocket_adapter.rs::Some("estop")]], [[engine/mantle/tetonic-server/src/main.rs::tokio::select!]].

Daemon shutdown rejects new work, cancels sessions and polls in-flight turns until zero or a five-second deadline, then main aborts the writer. Thus the code does not promise that every final notification is flushed before process exit. Sandbox cancellable execution has an explicit fail-closed default for unsupported backends. Backend cleanup behavior remains platform-specific. [[engine/litho/tetonicd/src/daemon.rs::pub async fn shutdown]], [[engine/litho/tetonicd/src/main.rs::writer.abort]], [[engine/core/tetonic-sandbox/src/backend/mod.rs::async fn execute_cancellable]].

## Trust and security boundaries

Application effect authorization combines policy evaluation, approvals, scoped capability issuance/consumption, workspace path checks and sandbox execution. Egress guard and secret scanning are additional boundaries around data leaving the process. These checks are not a single all-encompassing security layer: each caller must use the configured wrapper. The world server uses loopback restrictions and manifest action names but does not invoke the coding RuntimeActionBroker for world effects. [[engine/core/tetonic-runtime/src/action_broker.rs::evaluate_and_issue]], [[engine/core/tetonic-sandbox/src/backend/mod.rs::fn validate_working_directory]], [[engine/litho/tetonicd/src/main.rs::with_redactor]], [[engine/mantle/tetonic-server/src/main.rs::EmptyToolHost]], [[engine/core/tetonic-core/src/agent.rs::pub async fn run_in_world]].
...
Solid arrows are local synchronous callbacks or state access; dashed arrows are asynchronous client transports. Only the explicitly configured audit/run store is durable in this view. Server health remains available when payload capture is disabled. Evidence: [[engine/litho/tetonic-app/src/events.rs::pub]], [[engine/litho/tetonicd/src/main.rs::with_redactor]], [[engine/core/tetonic-runtime/src/brain.rs::pub struct SingleModelBrain]], [[engine/mantle/tetonic-server/src/observability.rs::pub fn record]], [[engine/mantle/tetonic-server/src/observability.rs::pub fn since]].
